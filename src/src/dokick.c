...
#else
		    pline("%sは%sにが，ダメージにならなかった．", xname(gold),
			      mon_nam(mtmp));
#endif
		    msg_given = TRUE;
		}
...
*/
		You("%ld%s分の損害を引きおこした！", loss, currency(loss));
...
#else
		You("%s%sを蹴ることができない！",
		    Hallucination ? "のぼせ上がっていては" :
		    trap->ttyp == WEB ? "くもの巣の中の" : "穴の底にある",
		    something);
#endif
		return 1;
	}
...
#else
	    pline("イェー！%sは%sの上を滑っていった．", Doname2(kickobj),
		surface(x,y));
#endif
...
char *buf;
{
	...
	else if (maploc->typ == IRONBARS) what = "鉄格子";
	else what = "何か妙なもの";
...
*/
		if (yn_function("乗騎を蹴る？", ynchars, 'y') == 'y') {
/*JP
...
*/
		You("ナップザックが重すぎて蹴りのためのバランスがとれない．");
		no_kick = TRUE;
...
*/
		default: Your("弱々しい蹴りではなんの効果もなかった．"); break;
		}
		return(1);
	}
	if (Levitation) {
...
#else
			    pline("%sが何%sか木から落ちてきた！"
				, xname(treefruit), numeral(treefruit));
#endif
			else
/*JP
...
#else
			    pline("%ld%sの%sが枝にひっかかった．",
			    	nfruit-nfall, numeral(treefruit), xname(treefruit));
#endif
