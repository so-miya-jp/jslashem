...
#else
				You("何かが%sの死体から飛び出してくるのを%s．", mon_nam(mtmp),
						Blind ? "感じた" : "見た");
#endif
				mtmp2->mpeaceful = 1;
				mtmp2->mtame = 0;
...
*/
			Sprintf(buf, "%sは%sを飲み込んだ．", Monnam(mtmp),
			    distant_name(otmp,doname));
		} else if (ecount == 2)
/*JP
			Sprintf(buf, "%s engulfs several objects.", Monnam(mtmp));
*/
			Sprintf(buf, "%sはいくつかの物を飲み込んだ．", Monnam(mtmp));
	...
	if (lifesave) {
		...
		if (visible) {
			...
			pline_The("medallion crumbles to dust!");
				...
				pline("%sはとても気分がよさそうだ！", Monnam(mtmp));
...
#else
	    	Sprintf(killer_buf, "%sの爆発", jtrns_mon(mdat->mname));
#endif
	    	killer = killer_buf;
...
int how;
{
	...
	{
	    if( *fltxt )
	      pline("%sは%sによって%s！", Monnam(mdef), fltxt,
		    nonliving(mdef->data) ? "倒された" : "殺された");
	    else
...
#else
	    if (!Hallucination) pline("よくない行為だった．．．");
	    else pline("シンジマッター！");
#endif
...
#else
		if (!Hallucination) You_hear("遠くで雷鳴を聞いた．．．");
		else pline("聴衆の喝采を浴びた！");
#endif
...
#else
		if (!Hallucination) pline("神は恐らくこれをよしとしないだろう．．．");
		else pline("シンジマッター！");
#endif
	}
...
#else
		if(!strcmp(string, "blast") || !strcmp(string,"息") ||
		   !strcmp(string,"風"))
		    shieldeff(u.ux, u.uy);
		pline("毒は効かなかったようだ．");
#endif
...
*/
	...
	if (!flags.mon_moving &&	/* should always be the case here */
		...
		pline_The("%s appear%s to be angry too...",
		      ...
		      got_mad == 1 ? "s" : "");
#else
		pline("%sも怒った．．．", jtrns_mon(q_guardian->mname));
#endif
	}
}
...
#else
				You("%sをそれに変化させることはできない．", mon_nam(mon));
#endif
			else break;
		} while(++tries < 5);
...
#else
		pline("あなたは%sに触れた．", mon_nam(u.usteed));
		Sprintf(buf, "%sに乗って", mon_nam(u.usteed));
#endif
	    	instapetrify(buf);
 	    }
...
#else
		/* + 4 は wereを削っているらしい・・・ */
		pline("%sは%sの姿になった！", oldname,
			is_human(mdat) ? "人間" : jtrns_mon(mdat->mname + 4));
#endif
	    else {
...
#else
	    pline("%sは%sになった！", oldname,
		  mdat == &mons[PM_GREEN_SLIME] ? "スライム" :
		  x_monnam(mtmp, ARTICLE_A, (char*)0, SUPPRESS_SADDLE, FALSE));
#endif
