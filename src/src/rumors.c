...
#else
		(strstri(rumor_buf, "占い") || strstri(rumor_buf, "なんてことだ"))
#endif
	    );
...
#else
		      "託宣の巫女は軽蔑したようにあなたの全てのお金を受けとり，告げた：" :
		      "託宣の巫女はしばらく冥想し，そして告げた：");
#endif
		else
/*JP
		    putstr(tmpwin, 0, "The message reads:");
*/
		    putstr(tmpwin, 0, "メッセージを読んだ:");
		putstr(tmpwin, 0, "");
...
*/
		pline("ここには神託を告げてくれるような者は誰もいない．");
		return 0;
	} else if (!oracl->mpeaceful) {
/*JP
		pline("%s is in no mood for consultations.", Monnam(oracl));
*/
		pline("託宣の巫女は神託を告げてくれる雰囲気ではない．");
		return 0;
#ifndef GOLDOBJ
	} else if (!u.ugold) {
#else
