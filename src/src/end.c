...
static NEARDATA const char *ends[] = {		/* "when you..." */
	"死んだ", "裏切られて死んだ", "窒息した", "毒におかされた", "餓死した", "溺死した",
	"焼死した", "溶岩に溶けた",
	"押し潰された", "石になった", "どろどろに溶けた",
	"虐殺された",
...
*/
...
#else
	{
	    /*JP
	      日本語では "為す術も無く" は "死んだ/殺された/石化した" 等の寸前で補う為，
	      if (multi)の処理をここに移動
	      またKILLED_SUFFIXの "に" が "為す術も無く" の前に入るようにここで処理
	    */
	    if (multi) Strcat(buf,"に為す術も無く");
	    else Strcat(buf,"に");
	    /*JP
...
*/
	...
	if (!done_stopprint) {
	    ...
	    c = ask ? yn_function("Do you want to see your conduct?",
				  ynqchars, defquery) : defquery;
#else
	    c = ask ? yn_function("どういう行動をとったか見ますか？",
				  ynqchars, defquery) : defquery;
#endif
	    if (c == 'y')
...
#else
		Your("魔除けは%s%s！",
		      !Blind ? "輝きはじめ" : "暖かくなりはじめ",
		      uamul->cursed ? "，崩れていく" : "た");
#endif
...
#else
		Sprintf(pbuf, "%s%s%sの墓", killer,
			killer_format == NO_KILLER_PREFIX ? "" :
			killer_format == KILLED_SUFFIX ? "殺された" :
			killed_by_prefix[how],
			plname);
#endif
...
*/
	if (u.uhave.amulet) Strcat(kilbuf, "魔除けを手に");
	else if (how == ESCAPED) {
	    if (Is_astralevel(&u.uz))	/* offered Amulet to wrong deity */
/*JP
		Strcat(kilbuf, " (in celestial disgrace)");
*/
		Strcat(kilbuf, "天上で恥辱を受け脱出した");
...
#else
	    Sprintf(pbuf, "%s%sの%s．．．", Goodbye(FALSE),
		   how != ASCENDED ?
		    (const char *) ((flags.female && urole.jname.f) ?
...
*/
		if (Is_astralevel(&u.uz)) where = "天上界にて";
/*JP
		Sprintf(pbuf, "You %s in %s", ends[how], where);
*/
		Sprintf(pbuf, "あなたは%s", jtrns_obj('d', where));
...
boolean ask;
{
	...
	c = ask ? yn_function("Do you want an account of creatures vanquished?",
			      ynqchars, defquery) : defquery;
#else
	c = ask ? yn_function("倒したモンスターの一覧を見ますか？",
			      ynqchars, defquery) : defquery;
#endif
	if (c == 'q') done_stopprint++;
	if (c == 'y') {
...
*/
			    Strcpy(buf, jtrns_mon(mons[i].mname));
			else
#if 0 /*JP*/
			    Sprintf(buf, "%d %s",
...
#else
			    Sprintf(buf, "%d体の%s",
				    nkilled, jtrns_mon(mons[i].mname));
#endif
		    }
		    putstr(klwin, 0, buf);
		}
...
*/
		Sprintf(buf, "%ld匹のモンスターを倒した．", total_killed);
		putstr(klwin, 0, buf);
	    }
...
boolean ask;
{
	...
	c = ask ? yn_function("Do you want a list of species genocided?",
			      ynqchars, defquery) : defquery;
#else
	c = ask ? yn_function("虐殺した種の一覧を見ますか？",
			      ynqchars, defquery) : defquery;
#endif
	if (c == 'q') done_stopprint++;
	if (c == 'y') {
...
*/
		...
		if (mvitals[i].mvflags & G_GENOD) {
			...
			Sprintf(buf, "%s%s",
				...
				mons[i].mname);
#else
			Sprintf(buf, "%s", jtrns_mon(mons[i].mname));
#endif
		    else
/*JP
