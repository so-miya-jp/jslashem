...
**	For SLASH'EM 0.0.7 Copyright (c) Masaki Miyaso, Shiganai Sakusha, 2004-2006
...
*/
STATIC_VAR const char are_blinded_by_the_flash[] = "まばゆい光で目がくらんだ！";
...
const char * const flash_types[] = {	/* also used in buzzmu(mcastu.c) */
	...
	"眠り光線",
	"死の指",
	"稲妻の閃光",    /* New spell & used for retribution */
	"毒霧",  /*WAC New spells acid + poison*/
	"酸の旋風",
	"",
	"",
#endif
	...
	"",
#else
	"魔力のブレス",	/* Dragon breath equivalents 20-29*/
	"炎のブレス",
	"冷気のブレス",
	"催眠ガスのブレス",
	"分解のブレス",
	...
	""
#else
	"魔法の爆風",        /* Megaspell equivalents must be 30-39 */
	"火の玉",             /*Should be same as explosion names*/
	"氷の玉",
	"",
	"",
	"雷玉",
...
#else
	"マーリンの知識と共に！",  /* Magic */ 
		/* マーリンはアーサー王の相談役である魔法使い */ 
	"ウィツロポチトリの情熱よ！", /* Fire */ 
		/* ウィツロポチトリはアステカ神話の太陽神 */ 
	"デメテルの悲しみよ！", /* Frost */
		/* デメテル - 彼女の娘であるペルセフォネがさらわれたとき，
		 * 	彼女の嘆きにより冬のようになってしまった．ギリシャ神話．
		 */
	"ヒュプノスの安らぎ", /* Sleep */ 
		/* ヒュプノス - ギリシャ神話の眠りの神 */
	"カーリーの憤怒", /* Disint */
		/* カーリーはヒンズー教の死と破壊の神 */
	"バルカンの炉より来たれ！", /* Lightning*/ 
		/* バルカンとはゼウスの雷の炉 */
	"ヨルムンガンドの牙より来たれ", /* Poison gas */ 
		/* 北欧神話の大蛇．  毒はラグナロクで空と海にわだかまる
		 */
	"狂気の化学者の怒り！", /* Acid */
#endif
	"",
...
#else
		    } else pline("%sが%s元気に%sようだ．", Monnam(mtmp),
			  (otyp == SPE_EXTRA_HEALING ||
			  otyp == WAN_EXTRA_HEALING) ? "とても" : "",
			  otmp->oclass == SPBOOK_CLASS ?
			    "なった" : "なりつつある");
#endif
		    makeknown(otyp);
...
*/
			    pline("%sの身体が肉に変わった！", name);
		    } else {
			if (canseemon(mtmp))
/*JP
			    pline("%s looks rather fleshy for a moment.",
*/
			    pline("%sの身体が一瞬非常に肉感的に見えた．",
				  name);
		    }
		} else
		    wake = FALSE;
...
*/
		    Norep("あなたは無力化してしまった未払い品を賠償しなければならない！");
...
*/
		    Norep("あなたは無力化してしまった商品を賠償しなければならない！");
		    bill_dummy_object(obj);
		} else
...
*/
	    pline("いくつかの%sが溶け，アイテムの山から%sが現れた！",
...
*/
	    getlin("どの種の怪物に変身しますか？[名前を入れてね]", buf);
...
*/
			pline("%sは霞がかかったように見えた．", Yname2(otmp));
	}
#endif
...
*/
			pline("%sは怒った！", Monnam(shkp));
			hot_pursuit(shkp);
		    }
/*JP
		} else Norep("%s is furious!", Monnam(shkp));
*/
		} else Norep("%sは激怒している！", Monnam(shkp));
		if (!carried(otmp)) {
		    if (costly_spot(u.ux, u.uy) && objroom == *u.ushops)
			bill_dummy_object(obj);
		    else
...
*/
	    pline("冗談言うなよ！こんなところでか？");
	    return; 
	} else if (Is_waterlevel(&u.uz)) { 
/*JP
	    You("better wait for the sun to come out.");
*/
	    You("太陽が出てくるまで待ったほうが賢明だろう．");
	    return;
	}
	switch (abs(type) % 10)
	{
...
#else
	static const char your[] = "あなた";	/* should be extern */
#endif
...
*/
				You("叫んだ．『%s』",yell_types[otyp - SPE_MAGIC_MISSILE]);
...
*/
	else pline("%sは%sに命中した%s", The(str),
		   mon_nam(mtmp), force);
}
...
*/
	...
	case ZT_POISON_GAS:
	    ...
	    poisoned("blast", A_DEX, "poisoned blast", 15);
#else
	    if (abs(type) < ZT_BREATH(0))
		poisoned("息", A_DEX, "毒吹き", 15);
	    else
...
#else
			    pline("%sの身体はあなたの%sの前で再結合されていく！",
				  s_suffix(Monnam(mon)), body_part(EYE));
#endif
/*JP
...
#else
	pay_for_damage(abstype == ZT_FIRE ?  "燃やす" :
		       abstype == ZT_COLD ?  "粉々にする" :
		       abstype == ZT_DEATH ? "粉砕する" : "破壊する", FALSE);
#endif
...
*/
		if (cansee(x,y)) Norep("くもの巣は炎に包まれた！");
		(void) delfloortrap(t);
