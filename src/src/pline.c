...
*/
	else if (mtmp->mtraitor) Strcat(info, ", 裏切り");
/*JP
...
*/
	pline("%sの状態 (%s)： Level %d  HP %d(%d)  Pw %d(%d)  AC %d%s",
		monnambuf,
		align_str(alignment),
		mtmp->m_lev,
		mtmp->mhp,
...
*/
	pline("%sの状態 (%s%sの%s徒)： Level %d  HP %d(%d)  Pw %d(%d)  AC %d%s",
#if 0 /*JP*/
		plname,
...
#else
	      plname,
		    (u.ualign.record >= 20) ? "敬虔な" :
		    (u.ualign.record > 13) ? "誠実な" :
		    (u.ualign.record > 8) ? "忠実な" :
		    (u.ualign.record > 3) ? "大げさな" :
		    (u.ualign.record == 3) ? "" :
		    (u.ualign.record >= 1) ? "迷い多き" :
		    (u.ualign.record == 0) ? "名ばかりの" :
					    "",
#endif
