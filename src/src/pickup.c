 ...
 */
/*JP CHECK: 0.0.7E7 での呼び出し元
pickup.c:580:("拾う")	if (!query_classes(oclasses, &selective, &all_of_a_type,
pickup.c:2798:("取り出す")    if (query_classes(select, &one_by_one, &allflag,
pickup.c:2900:("入れる")	if (query_classes(select, &one_by_one, &allflag, "入れる",
*/
#ifndef GOLDOBJ
STATIC_OVL boolean
...
*/
		n = query_objlist("何を拾いますか？", objchain,
...
*/
			"祝福されているとわかっているもの", MENU_UNSELECTED);
	}
	if (do_cursed) {
		invlet = 'C';
		any.a_void = 0;
...
*/
			"呪われているとわかっているもの", MENU_UNSELECTED);
	}
	if (do_uncursed) {
		invlet = 'U';
		any.a_void = 0;
...
*/
			"呪われていないとわかっているもの", MENU_UNSELECTED);
	}
	if (do_buc_unknown) {
		invlet = 'X';
		any.a_void = 0;
...
*/
	...
	*wt_after = wt;
	...
	suffx  = "";
#else
	prefx1 = (obj->quan == 1L) ? "重すぎて" : "その1つですら重すぎて";
#endif
    }
#if 0 /*JP*/
...
#else
		msg = (next_encumbr > HVY_ENCUMBER) ? overloadmsg :
			(next_encumbr > MOD_ENCUMBER) ? nearloadmsg :
			moderateloadmsg;
...
*/
	...
	if (obj == uchain) {    /* do not pick up attached chain */
...
#else
			pline("%sに触れるのは致命的な間違いだ．", kbuf);
			Sprintf(kbuf, "%sの死体に触れて", killer_cxname(obj, TRUE));
#endif
			instapetrify(kbuf);
		    return -1;
		}
		...
		pline_The("scroll%s %s to dust as you %s %s up.",
			...
			(obj->quan == 1L) ? "it" : "them");
#else
		pline("%s巻物はあなたが%s塵となってしまった．",
			(obj->quan == 1L) ? "その" : "それらの",
			telekinesis ? "確保すると" : "拾い上げると");
#endif
...
encumber_msg()
{
	...
	case 2: You("荷物の釣合をとり直したが，動きにくい．");
		break;
	case 3: You("荷物の重みで%s．動くのが非常にきつい．",
		    stagger(youmonst.data, "のたうちまわった"));
		break;
...
#else
	if (!get_adjacent_loc("どの方向を調べる？", "調べるものがない",
			u.ux, u.uy, &cc)) return 0;
#endif
...
*/
		    You("鞍は外せなかった．どうやら%sにくっついてしまっているようだ．",
...
#else
		    pline("%sが爆発に巻き込まれるのが見えた！", mon_nam(mtmp));
#endif
		mtmp->mhp -= d(6,6);
		if (mtmp->mhp < 1) {
		    if (!bomb->yours) 
...
#else
			monkilled(mtmp, silent ? "" : "爆発", AD_PHYS);
#endif
...
#else
	    	losehp(d(6,6), "爆発物運搬中に事故で", KILLED_BY);
#endif
		break;
	    case OBJ_FLOOR:
...
#else
			    pline("あなたの周囲に水柱が立ち昇った．");
#endif
#if 0 /*JP*/
			else pline("A bag explodes under your %s!",
			  makeplural(body_part(FOOT)));
#else
			else pline("あなたの%s元で鞄が爆発した！",
			  makeplural(body_part(FOOT)));
#endif
		    } else if (cansee(x, y))
#if 0 /*JP*/
...
*/
	      pline("どういうわけかその石はあなたから離れようとしない．");
		return 0;
...
*/
		pline("%sは大きすぎて%sに詰め込むことはできない．", buf,
...
#else
		    pline("%sに触れるのは致命的な間違いだ．", kbuf);
		    Sprintf(kbuf, "%sの死体に触れて", killer_cxname(uwep, TRUE));
#endif
		    instapetrify(kbuf);
		    return -1;
		}
...
*/
		verbalize("卑劣なやつめ！すぐにつるはしを持って消えな！");
...
#else
	You("%sが消滅したのを%s！", doname(item), Blind ? "感じた" : "見た");
#endif
...
*/
	    You("%sがあなたの%sをくすぐった．", something, body_part(FOOT));
	else
/*JP
	    pline("%s inside the box is still alive!", Monnam(livecat));
*/
	    pline("箱のなかの%sはまだ生きている！", Monnam(livecat));
...
#else
	pline("箱の中の%sは死んでいる！",
	    jtrns_mon(Hallucination ? rndmonnam() : "housecat"));
#endif
    }
    box->owt = weight(box);
    return;
...
*/
	...
	if (!cnt)
	    ...
	    Sprintf(emptymsg, "%s is %sempty.", Yname2(obj),
		    quantum_cat ? "now " : "");
#else
	    Sprintf(emptymsg, "%sは%s空っぽだ．", Yname2(obj),
		    quantum_cat ? "今は" : "");
#endif
