...
*/
                        if(yn("本当に保存する？") == 'n') saverestore = TRUE;
...
*/
		    pline("セーブディスクの空き容量が足りません．");
/*JP
		    pline("Require %ld bytes but only have %ld.", needed, fds);
*/
		    pline("%ldバイト必要ですが%ldバイトしか空きがありません．", needed, fds);
		}
		flushout();
		(void) close(fd);
...
*/
			    "Level #0のpid(%d)が我々のpid(%d)と一致しない！",
			    hpid, hackpid);
		    pline("%s", whynot);
		    killer = whynot;
		    done(TRICKED);
