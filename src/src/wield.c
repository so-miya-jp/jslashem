...
*/
		You("すでに何も%sに持っていない．", body_part(HAND));
...
*/
		...
		pline("%s%s %s to your %s!", tmp, aobjnam(wep, "weld"),
			...
			bimanual(wep) ?
				...
				: body_part(HAND));
#else
	        pline("%sはあなたの%s%sに癒着してしまった！",
		      xname(wep), bimanual(wep) ? "両" : "", body_part(HAND));
#endif
		wep->bknown = TRUE;
	    } else {
...
#else
	    pline("武器があなたの%s%sに癒着しているので，%sを使えない．",
		  bimanual(uwep) ? "両" : "", hand, xname(obj));
#endif
	} else {
/*JP
	    You_cant("do that.");
...
*/
	    what = uwep && uswapwep ? "二刀流" : "二本以上の武器";
	    if (cantwield(youmonst.data))
/*JP
		pline("Don't be ridiculous!");
*/
...
*/
		    Strcpy(buf, disallowed_by_race ? urace.j : urace.adj);
...
#else
		pline("%sは両手持ちによる戦闘ができない．",
			upstart(makeplural(urace.j)));
#endif
...
#else
		what = uwep && uswapwep ? "二刀流はできない" : "二本以上の武器は使えない";
#endif
	    else {
/*JP
		sprintf(buf, "fight with two %s", makeplural(body_part(HAND)));
*/
		sprintf(buf, "両%sによる戦闘はできない", makeplural(body_part(HAND)));
		what = buf;
	    }
/*JP
	    You_cant("%s while wearing a shield.", what);
*/
	    You("盾を持っている間は%s．", what);
	}
...
/*JP
		    You("are empty %s.", body_part(HANDED));
*/
		    You("何も%sにしていない．", body_part(HANDED));
		    unweapon = TRUE;
		} else
/*JP
		    You("switch to your right %s.", body_part(HAND));
*/
		    You("右%sに持ち替えた．", body_part(HAND));
		if (uswapwep)
...
*/
		    You("この%sではもう武器を保持していくことができない．",
			    body_part(HAND));
		else
/*JP
		    You("can no longer use two %s to fight.",
*/
		    You("もう両%sでの戦闘をすることはできない．",
			    makeplural(body_part(HAND)));
		if (uswapwep)
		    unwield(uswapwep, TRUE);
		u.twoweap = FALSE;
...
*/
		...
		pline("%s's %s%s!", Monnam(victim),
		    ...
		    erosion ? " further" : "");
#else
		pline("%sの%sは%s%sた！", Monnam(victim),
			  xname(target),
			  erosion+1 == MAX_ERODE ? "完全に" :
			  erosion ? "さらに" : "",
			  acid_dmg ? "腐食し" : "錆び");
		...
		if (victim == &youmonst)
			...
			acid_dmg ? "corroded" : "rusty");
#else
		    pline("%sの%sは完全に%sてしまっている%s．", Monnam(victim),
			xname(target),
			acid_dmg ? "腐食し" : "錆び",
			Blind ? "ようだ" : "");
#endif
