...
*/
	    You("もう%sの上に座っている．", mon_nam(u.usteed));
	    return (0);
	}
#endif
...
*/
	    if ((!Sick) && (u.uhs > 0)) You("する必要がない．．．");
	    else {
#if 0 /*JP*/
			if (Role_if(PM_BARBARIAN) || Role_if(PM_CAVEMAN)) You("miss...");
			else You("grunt.");
#else
			if (Role_if(PM_BARBARIAN) || Role_if(PM_CAVEMAN)) You("外した．．．");
			else You("うなった．");
#endif
...
*/
	    if (!Cold_resistance) pline("氷を冷たく感じた．");
...
*/
	static const char mal_aura[] = "邪悪なオーラが%sを包むのを感じた．";
