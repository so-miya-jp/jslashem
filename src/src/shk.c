...
*/
	Strcpy(kopname, jtrns_mon("Keystone Kop"));
...
#else
		pline("%sは助っ人を呼んだ！", noit_Monnam(shkp));
#endif
		mesg_given = TRUE;
	    }
	    wakeup(mt);
...
*/
	    verbalize("透明なお客さまのご入店はお断りしています！");
	    return;
	}
#else /* BLACKMARKET */
/*JP
...
*/
	      verbalize("人間じゃないお客さまのご入店はお断りしています！");
	      return;
	}
#endif /* BLACKMARKET */
...
*/
	    verbalize("%s！わざわざ%sの%sに何しに戻ってきた？！",
		      plname,
...
*/
	    pline("%sは万引き犯への悪態を呟いている．", shkname(shkp));
	} else {
#if 0 /*JP*/
...
boolean verbosely;
{
	if (!shkp->mcanmove || shkp->msleeping) {
		...
		pline("%sは%s．", Monnam(shkp),
		      shkp->msleeping ? "目を覚ました" : "ふたたび動けるようになった");
#endif
	    shkp->msleeping = 0;
	    shkp->mfrozen = 0;
	    shkp->mcanmove = 1;
...
*/
		    pline("%sは満足すると，突然消えた！", shk_nam);
	} else if(wasmad)
/*JP
		pline("%s calms down.", Monnam(shkp));
*/
		pline("%sは怒りを収めた．", Monnam(shkp));
...
*/
			if (stashed_gold) pline("しかし，あなたにはヘソクリがある！");
#else
			pay(umoney, shkp);
/*JP
...
*/
		    pline("%sはお金ではなく，あなたの流血を望んでいる！", Monnam(shkp));
#ifndef GOLDOBJ
		    if(u.ugold < ltmp/2L ||
...
*/
		    pline("%sはお金ではなく，あなたから皮を剥ぐことを望んでいる！",
			  Monnam(shkp));
#ifndef GOLDOBJ
		    if(u.ugold < 1000L) {
			if (!u.ugold)
...
*/
			...
			mhim(shkp));
#else
		    You("金貨1000枚を%sに手渡してなだめようとした．",
			x_monnam(shkp, ARTICLE_THE, "怒った", 0, FALSE));
#endif
		    pay(1000L,shkp);
...
*/
			pline("しかし%sの怒りに火を注いだだけだった．", mon_nam(shkp));
		}
		return(1);
	}
	if(shkp != resident) {
...
#else
		    else Strcpy(sbuf,"店内で拾った金貨や使った商品の使用料として，");
#endif
/*JP
		} else Strcat(sbuf, "for the use of merchandise.");
*/
		} else Strcpy(sbuf, "使った商品の使用料として，");
#if 1 /*JP*/
		Sprintf(eos(sbuf), "%sに%ld%s分の借りがある．",
			shkname(shkp), dtmp, currency(dtmp));
#endif
		pline(sbuf);
#ifndef GOLDOBJ
...
#else
	    Sprintf(qbuf,"%sは%ld%sです．買いますか？", quan == 1L ?
#endif
...
#else
	shk_names_obj(shkp, obj, consumed ?
			"%sの使用料として金貨%ld枚%sを支払った%s．" :
			"%sを金貨%ld枚%sで買った%s．", ltmp, "");
#endif
...
#else
		pline("%sは%sあなたの死体を見て%s%s．",
		      Monnam(shkp),
		      (!shkp->mcanmove || shkp->msleeping) ? "目をさますと" : "",
		      !rn2(2) ? "首を振り，" : "",
...
*/
		if (!is_human(youmonst.data)) Strcat(buf, "生物どの");
		else
/*JP
		    Strcat(buf, (flags.female) ? " lady" : " sir");
*/
		    Strcat(buf, (flags.female) ? "ご婦人" : "旦那さま");
	    }
	    if(ininv) {
		long quan = obj->quan;
		...
		pline("%sは%s%ld%sだ．",
			The(xname(obj)),
			(obj->quan > 1L) ? "それぞれ" : "", ltmp, currency(ltmp));
#endif
	} else if(!silent) {
#if 0 /*JP*/
...
#else
		    shk_names_obj(shkp, obj, (sell_how != SELL_NORMAL) ?
			  "%sを%ldゴールド分の預け金で受けとった%s．" :
			  "%sを渡し，%ldゴールド分の預け金を得た%s．",
				  tmpcr, "");
#endif
		    eshkp->credit += tmpcr;
		    subfrombill(obj, shkp);
...
#else
		    Sprintf(qbuf,
			    "%sはあなたの%s%sに金貨%ld枚の値%s．売りますか？",
			    Monnam(shkp), cxname(obj),
			    (!ltmp && cltmp) ? "の中身" : "",
			    offer, 
...
*/
		...
		if (cansee(x, y)) {
		    ...
		    pline("%s nimbly%s catches %s.",
			  ...
			  the(xname(obj)));
#else
		    pline("%sはすばやく%s%sをつかまえた．",
			  Monnam(shkp),
			  (x == shkp->mx && y == shkp->my) ? "" : "近づいて",
			  the(xname(obj)));
#endif
		    if (!canspotmon(shkp))
			map_invisible(x, y);
...
#else
			    verbalize("%s！わたしは%sを探しています．",
				    Hello(shkp, TRUE), eshkp->customer);
#endif
				    eshkp->following = 0;
			    return(0);
			}
...
#else
	    verbalize("注意してください%s，床を掘り抜いて落ちますよ．",
		flags.female ? "ご婦人" : "旦那さま");
#endif
	    else
#if 0 /*JP*/
...
*/
		pline("%sはあなたのナップザックを%s！", shkname(shkp), grabs);
...
#else
	boolean ni_wo = !strcmp(dmgstr, "穴をあける");		/* wand */
	boolean dugwall = (ni_wo || !strcmp(dmgstr, "傷つける")); /* pick-axe */
#endif
...
#else
	Sprintf(qbuf,"「おい！%ld%sの損害だ！」払いますか？",
#endif
...
*/
	    ...
	    Sprintf(price, "%s%ld%s", 
		    otmp->quan > 1L ? "それぞれ" : "", cost, currency(cost));
#endif
	}
#if 0 /*JP*/
...
#else
    "%s曰く『ショッピングモールってやつは僕にとっては頭痛のタネだったよ．』",
    "%s曰く『落ち着いてやろう．理路整然と考えるんだ．』",
    "%s曰く『物事は一度に一つずつ解決していくべきだ．』",
    "%s曰く『甘いコーヒは好きじゃない．．．コロンビアン・スプレモをたのむ．』",
    "%sは開発チームの統一見解を定めることは困難だ，と述べた．",
    "%sは自らの神に奉仕するものは成功することに気づいた，と述べた．",
    "%s曰く『私から盗もうなんて思わないことだ．天界に友達がいるからね！』",
    "%s曰く『君はおそらく，将来この店の何かを必要とすることになるだろう．』",
    "%sは死の谷は言わばゲートウェイだろうとコメントを述べた．"
#endif
};
...
#else
		pline("%sは最近どこかに放置された店はなかったかとたずねた．",
#endif
		      Monnam(shkp));
...
#else
		pline("%sはあなたには%ld%sの貸しがあると念を押した．",
		      shkname(shkp),
		      eshk->debit, currency(eshk->debit));
#endif
	else if (eshk->credit)
...
#else
		pline("%sは最近強盗されたことについて愚痴をこぼした．", shkname(shkp));
#endif
#ifndef GOLDOBJ
...
*/
		pline("%sは商売がうまくいってないと愚痴をこぼした．", shkname(shkp));
#ifndef GOLDOBJ
...
*/
		pline("%sは商売がうまくいっていると言った．", shkname(shkp));
...
*/
		verbalize("この種類の品を扱った事は無いのだが．．．やってみよう．");
	}
...
*/
		ident_type = yn_function("通常サービス[b] 又は特殊サービス [p]",
		     ident_chars, '\0');
...
*/
		pline("%sは強欲な笑みをもらした．．．", mon_nam(shkp));
		mult = 1;
...
*/
		pline("%sが何か言ったようだったがあなたは忘れてしまった．", mon_nam(shkp));
		return;
	    }
	}
...
*/
		    pline("あなたの血走った%sに気を取られた店主は，",
			makeplural(body_part(EYE)));
		    pline("何故かその品を祝福してしまった！");
		    bless(obj);
		}
		else
		{
...
*/
		verbalize("終わったよ．これでもう手にしても大丈夫だ！");
		uncurse(obj);
	}
}
...
*/
	/* 出力されるのはoc_wsdamとoc_wldam = 武器の基本最大ダメージ [Sakusha]*/
	"小型の相手に最大%s，大型の相手に最大%sのダメージを与えられるな．";
...
*/
		verbalize("オーケイ，%s．商品を見ていってくれ．", slang);
		guesswork = FALSE;
	}
	else
	{
...
*/
		pline("数字があなたのあたまのなかですっかりごっちゃになってしまった．");
		return;
	}
	else if (Hallucination)
	{
...
*/
		You("%sがこれは文字通り「相手を叩き殺す」だろうと言うのを聞いた．",
			mon_nam(shkp));
		return;
	}
...
*/
			verbalize("済まない%s，私にはわからない．", slang);
			break;
...
*/
static const char we_offer[] = "我々は最高のサービスを提供しております！";
static void
shk_weapon_works(slang, shkp)
char *slang;
struct monst *shkp;
...
*/
	pline("%sは強欲ににやりと笑った．．．", mon_nam(shkp));
...
*/
	    verbalize("%sに触れないでくれよ！", xname(obj));
...
*/
		verbalize("これはまたひどい状態ですな，%s．", slang);
...
*/
	    verbalize("武器を壊したりはしませんぜ，もし壊したら御代は返します！");
	    /*
...
*/
		Your("%sは突然振動した！", xname(obj));
	    else if (Hallucination)
/*JP
		Your("%s to evaporate into thin air!", aobjnam(obj, "seem"));
*/
	    /* ...壊れてくれれば面白かったのだが絶対に壊れないようなのでここで意訳 */
		pline("「クホホホホホ！」あなたの%sは蒸発してしまった！", xname(obj));
...
*/
	    verbalize("どんな毒塗り%sが出来上がるか想像してみてくれ！", xname(obj));
...
*/
		add_menu(tmpwin, NO_GLYPH, &any , 'r', 0, ATR_NONE, "耐火/腐食防御", MENU_UNSELECTED);
	any.a_int = 2;
	if (ESHK(shkp)->services & (SHK_SPECIAL_B))
/*JP
		add_menu(tmpwin, NO_GLYPH, &any , 'e', 0, ATR_NONE, "Enchant", MENU_UNSELECTED);
*/
		add_menu(tmpwin, NO_GLYPH, &any , 'e', 0, ATR_NONE, "魔力付与", MENU_UNSELECTED);
/*JP
	end_menu(tmpwin, "Armor-works:");
*/
	end_menu(tmpwin, "防具加工:");
	n = select_menu(tmpwin, PICK_ONE, &selected);
	destroy_nhwindow(tmpwin);
...
*/
		     verbalize("あなたは鋼鉄の男と呼ばれるようになるだろうな！");
...
*/
		if (obj->oeroded > 2) verbalize("うひゃ！ こいつは滅茶苦茶だな！");
...
*/
			You("%sを身に着ける方法を忘れてしまった！", xname(obj));
		else if (Hallucination)
/*JP
			You("mistake your %s for a pot and...", xname(obj));
*/
			You("%sをポットと間違えた．そして．．．", xname(obj));
...
*/
		verbalize("どんな攻撃もあなたに当たらなくなりますよ．");
...
*/
		if (Hallucination) Your("%sはへこんだように見える．", xname(obj));
...
*/
			type = yn_function("通常サービス[b] 又は特殊サービス [p]",
					ident_chars, '\0');
			if (type == '\0') return;
...
*/
		pline("%sは充填が終わったと言って，あなたを出入り口に押していった．",
			Monnam(shkp));
		return;
	}
...
*/
			verbalize("あなたは望む物全てを手に入れられるのでしょうな．");
			verbalize("．．．私にお金を融資してはくれませんかね？");
#ifndef GOLDOBJ
			shkp->mgold += u.ugold;
			u.ugold = 0;
#else
...
*/
	/* 日本語には複数のsは無い */
        Sprintf(sbuf, "%ldゴールドかかります．宜しいですか？", charge);

	if ( yn(sbuf) != 'y' ) {
/*JP
		verbalize("It's your call, %s.", slang);
*/
		verbalize("あなたが頼んだんですよ，%s．", slang);
		return(FALSE);
	}
...
*/
		verbalize("これは現金払いでないと駄目ですぜ，%s，であんたは金を持ってない！",
			slang);
		return(FALSE);
	}
...
*/
		Sprintf(buf, "%sに対する借り", noit_mon_nam(shkp));
		if (shkp == ushkp) {
		    Strcat(buf, " (here)");
		    ushkp = NULL;
		}
...
*/
		Sprintf(buf, "あなたは%sには借りがない．", noit_mon_nam(shkp));
		putstr(win, 0, buf);
	    }
	    if (special)
		break;
