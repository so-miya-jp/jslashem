...
*/
	    if (transform_msg) pline("%sは震えた．", Monnam(mtmp));
...
*/
	    if (canseemon(mtmp) && !silent) pline("%sは震えた．", oldname);
	    mondead(mtmp);
	    return;
	}
...
*/
	    verbalize("ピンを抜いてしまったため，賠償しなければならない！");
...
*/
		    	losehp(d(2,5), "爆発物運搬中に事故で", KILLED_BY);
		    	break;
		    case OBJ_FLOOR:
...
*/
		pline("%sは%s揺らめいた．", Yname2(obj), tailer);
		break;
	    case OBJ_FLOOR:
/*JP
		You("see %s flicker%s.", an(xname(obj)), tailer);
*/
		You("%sが%s揺らめくのが見えた．", an(xname(obj)), tailer);
		break;
	}
}
...
*/
				    pline("%s%sが暗くなった！",whose, xname(obj));
				    break;
				case OBJ_FLOOR:
/*JP
				    You("see %s dim!", an(xname(obj)));
*/
				    You("%sの刃が暗くなるのを見た！", xname(obj));
				    break;
			    }
			} else {
/*JP
...
*/
			    pline("%s%sは停止した．",whose, xname(obj));
			    break;
			case OBJ_FLOOR:
/*JP
			    You("see %s deactivate.", an(xname(obj)));
*/
			    You("%sが停止するのを見た．", xname(obj));
			    break;
		}
	    } else {
/*JP
...
*/
		pline("%sのうなりが聞こえなくなった．", xname(obj));
	    }
	}
