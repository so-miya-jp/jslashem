...
*/
	      pline("誰かがあなたから盗もうとしたが，何も盗むものがなかった．");
	    else
/*JP
	      pline("%s tries to rob you, but there is nothing to steal!",
*/
	      pline("%sはあなたから盗もうとしたが，何も盗むものがなかった！",
		Monnam(mtmp));
 ...
 cant_take:
		pline("%s tries to %s your %s but gives up.",
		      ...
		      (otmp->owornmask & W_ARMOR) ? equipname(otmp) :
		       cxname(otmp));
#else
		static const char * const how[] = { "盗も","ひったくろ","掴も","奪お" };
 cant_take:
		pline("%sは%sを%sうとしたが諦めた．",
		      Monnam(mtmp),
		      (otmp->owornmask & W_ARMOR) ? equipname(otmp) :
		...
		case ARMOR_CLASS:
			...
			if (do_charm) {
			    ...
			    pline("%sはあなたを魅了した．あなたは喜んで%sを%s．",
				  !seen ? mhe(mtmp) : Monnam(mtmp),
				  equipname(otmp),
				  curssv ? "持って行かせた" :
				  slowly ? "外しはじめた" : "外して渡した");
#endif
			}
			else
#if 0 /*JP*/
...
#else
			    if(curssv){
				pline("%sはあなたを誘惑して，あなたが%sを外すのを手伝った．",
				  !seen ? mhis(mtmp) : Adjmonnam(mtmp,
				  mtmp->female ? "美しい" : "りりしい"),
				  equipname(otmp));
			    } else {
				pline("あなたは%sに誘惑され，%sを%s．",
				  !seen ? mhis(mtmp) : Adjmonnam(mtmp,
				  mtmp->female ? "美しい" : "りりしい"),
				  equipname(otmp),
				  slowly ? "外しはじめた" : "外した");
...
*/
	pline("%sは%sを盗んだ．", named ? "それ" : Monnam(mtmp), doname(otmp));
