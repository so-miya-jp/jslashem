	    ...
	    case TROUBLE_BLIND:
			...
			Your("%s feel%s better.",
			 (num_eyes == 1) ? eye : makeplural(eye),
			     (num_eyes == 1) ? "s" : "");
#else
			Your("%sは回復してきたようだ．", body_part(EYE));
#endif
		    u.ucreamed = 0;
...
*/
	    pline("突然，一筋の稲妻があなたに命中した！");
	    if (Reflecting) {
		shieldeff(u.ux, u.uy);
/*JP
		if (Blind) pline("For some reason you're unaffected.");
*/
		if (Blind) pline("なぜかあなたは影響を受けない．");
		else
/*JP
...
*/
		You("その%s輝きをしばらく浴び続けた．．．", NH_BLACK);
/*JP
		godvoice(resp_god, "You have further angered me!");
*/
		godvoice(resp_god, "きさまは私をさらに怒らせた！");
	    }
...
*/
	    Your("%sが目にも止まらぬ速さで振り回された！", xname(obj));
	    obj->dknown = TRUE;
...
#else
			Your("%sは%sやわらかく輝%s%s．", xname(uwep), 
			     jconj_adj(hcolor(NH_AMBER)),
			     *repair_buf ? "き，": "いた", repair_buf);
#endif
/*JP
...
#else
			Your("%sは%sやわらかなオーラにつつまれ%s%s．",
			     xname(uwep), hcolor(NH_LIGHT_BLUE),
			     *repair_buf ? "，" : "た", repair_buf);
#endif
/*JP
...
#else
			Your("%sは%sように%s！", xname(uwep), repair_buf,
			     Blind ? "感じる" : "見える");
#endif
		}
		update_inventory();
...
*/
		Your("献上物から翼とプロペラが生え，うなりながら飛んでいった！");
		break;
	    case 1:
/*JP
		Your("sacrifice puffs up, swelling bigger and bigger, and pops!");
*/
		Your("献上物は風船のように膨れ，どんどん大きくなり，はじけた！");
		break;
	    case 2:
/*JP
		Your("sacrifice collapses into a cloud of dancing particles and fades away!");
*/
		Your("献上物は踊り狂う小片となって崩れ落ち，どこかに行ってしまった！");
		break;
	}
...
*/
		pline("%s", Blind ? "あなたは邪悪なくすくす笑いを聞いた！" :
		 "悪臭のする毒気があなたの周りに集まった！");
	   break;
	}
/*JP
...
*/
	godvoice(u.ualign.type, "我がしもべは必ずや役立つであろう！");
	return;
    }
}
...
*/
    if (u.uhp > (u.uhpmax / 10)) godvoice(u.ualign.type, "我がしもべは必ずや役立つであろう！");
    else godvoice(u.ualign.type, "我がしもべはそなたを救うであろう！");
}
...
*/
	pline("%sはあなたを%s見つめた．", Monnam(mtmp), woodchuck ? "軽蔑するようなまなざしで" : "つくづくと");
...
/*JP
		    pline_The("altar is stained with %s blood.",
			    races[race].adj);
*/
		pline("祭壇は%sの血で汚れている．", races[race].j);
		else
...
*/
				You("激しい怒りが自分に向けられているのを感じた！", a_gname());
...
#else
		    pline("%sは%s見える．", u_gname(),
			  Hallucination ? "いかして" : "ちょっと和らいだように");
#endif
...
STATIC_PTR int
prayer_done(VOID_ARGS)		/* M. Stephenson (1.0.3b) */
{
...
*/
	pline("%sに祈りを込めて，あなたは秘密の聖句を吟じた．", u_gname());
	exercise(A_WIS, TRUE);
