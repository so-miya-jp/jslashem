...
#else
	if (yours) pline("%sは握ろうとするとするりと抜けた！", xname(obj));
#endif
	return 0;
    }
...
#else
	Sprintf(buf, "  %s [%s %s]", jtrns_obj('A', artiname(m)),
		align_str(artilist[m].alignment), simple_typename(otyp));
#endif
...
#else
	pline("魔力を吸いとる刃が%sを%sた！", hittee, verb);
#endif
...
#else
	    if (do_stun && do_confuse) Strcat(buf, "よろめき，");
	...
	if (otmp->oartifact == ART_SERPENT_S_TONGUE) {
...
#else
	    pline_The("ねじくれた刃が%sを毒に冒した！",
		    youdefend ? "あなた" : mon_nam(mdef));
#endif
...
#else
		    pline("%sには毒は効かないようだ．．", Monnam(mdef));
#endif
		return TRUE;
	    }
	    switch (rnd(10)) {
       ...
       if (otmp->oartifact == ART_DOOMBLADE && dieroll < 6) {
...
#else
		You("%sを%sに深く突き刺した！", jtrns_obj('A', artilist[ART_DOOMBLADE].name),
#endif
			mon_nam(mdef));
	    else
#if 0 /*JP*/
		pline("%s plunges the Doomblade deeply into %s!",
			Monnam(magr), hittee);
#else
		pline("%sは%sを%sに深く突き刺した！",
			Monnam(magr), jtrns_obj('A', artilist[ART_DOOMBLADE].name), hittee);
#endif
	    *dmgptr += rnd(4) * 5;
	    return TRUE;
       }
...
#else
			pline("%sが%sを真っ二つにした！", wepdesc, mon_nam(mdef));
#endif
			otmp->dknown = TRUE;
			return TRUE;
		} else {
...
#else
		wepdesc = jtrns_obj('A', artilist[ART_VORPAL_BLADE].name);
#endif
		if (!youdefend) {
...
#else
				pline("%sは%sの%sを空しく通り抜けた．", wepdesc,
#endif
...
#else
			pline("%sは%s！", The(distant_name(otmp, xname)), Blind ?
				"耳を聾せんばかりの音をたてた" :  "光り輝いた");
#endif
#if 0 /*JP*/
			pline("It strikes %s!", hittee);
#else
			pline("それは%sに直撃した！", hittee);
#endif
		    }
...
*/
		pline("%sが瞬時に明るく輝いた！", The(xname(obj)));
	    else
/*JP
		pline("%s grows warm for a second!", The(xname(obj)));
*/
		pline("%sが一時的に暖かくなった！", The(xname(obj)));
...
*/
			pline("%sは輝きの中で焼け焦げた！", Monnam(mtmp));
		    else
			unseen++;
...
*/
		You_hear("激痛を訴える%s叫びを聞いた！", unseen > 1 ? "複数の" : "");
	    break;
	case DEATH_GAZE:
	    if (u.uluck < -9) {
/*JP
...
*/
		    pline("%sは苦痛の金切り声を上げた！", Monnam(mtmp));
		    mtmp->mhp /= 3;
		    if (mtmp->mhp < 1) mtmp->mhp = 1;
		}
	    }
