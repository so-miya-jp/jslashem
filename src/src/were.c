...
register struct monst *mon;
{
	...
	if (is_human(mon->data)) {
		...
		if (flags.soundok && !canseemon(mon)) {
		    ...
		    case PM_HUMAN_WEREPANTHER:	howler = "ヒョウ";	break;
		    case PM_HUMAN_WERETIGER:	howler = "虎";		break;
#endif
...
*/
			You_hear("月夜に%sが吠えるのを聞いた．", howler);
		}
	    }
	...
	if (purify) {
		...
		if (in_wereform && Unchanging) {
...
#else
		    killer = "怪物の姿から戻れないまま浄化されて";
		    pline_The("浄化の力は致命的だった．．．");
#endif
		    done(DIED);
		} else {
#if 0 /*JP*/
		    ...
		    losehp(u.uhp - (u.uhp > 10 ? rnd(5) : 1), "purification",
			    KILLED_BY);
#else
		    losehp(u.uhp - (u.uhp > 10 ? rnd(5) : 1), "浄化されて",
			    KILLED_BY);
#endif
		}
		return;
...
#else
		(!Polymorph_control || yn("獣形態のままでいる？") == 'n'))
#endif
	    rehumanize();
}
