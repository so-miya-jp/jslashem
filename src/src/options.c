...
static struct Comp_Opt
{
	...
	{ "altkeyhandler", "代替のキーハンドラ", 20, DISP_IN_GAME },
#endif /*JP*/
#if 0 /*JP*/
	...
	{ "gender",   "ゲーム開始時の性別(male または female)",
						8, DISP_IN_GAME },
	{ "ghoulname",  "冒険を供にする(最初の)グールの名前 (例 ghoulname:カスパー)",
						PL_PSIZ, DISP_IN_GAME },
#endif /*JP*/
#if 0 /*JP*/
...
#else /*JP*/
	{ "name",     "あなたの名前 (例 name:Merlin-W)",
						PL_NSIZ, DISP_IN_GAME },
	{ "number_pad", "テンキーを使用する", 1, SET_IN_GAME},
#endif /*JP*/
#if 0 /*JP*/
...
# endif
	...
	{ "pickup_burden",  "拾う前に確認する最大荷重レベル",
						20, SET_IN_GAME },
	{ "pickup_types", "自動的に拾うアイテムのシンボル",
						MAXOCLASSES, SET_IN_GAME },
	{ "player_selection", "キャラクター選択にダイアログや確認を使う",
						12, DISP_IN_GAME },
#endif
#if 0 /*JP*/
	...
	{ "wolfname",  "冒険を供にする(最初の)狼の名前 (例 wolfname:ビースト)",
						PL_PSIZ, DISP_IN_GAME },
#endif /*JP*/
#if 1 /*JP*/
...
*/
	pline("%sは%sまたは%sの中でのみ設定できます．", optname,
...
*/
	    raw_printf("%s内のOPTIONSに文法エラー: %s．", configfile, opts);
	else
/*JP
	    raw_printf("Bad syntax in %s: %s.", NETHACK_ENV_OPTIONS, opts);
*/
	    raw_printf("%s内で文法エラー: %s．", NETHACK_ENV_OPTIONS, opts);
...
*/
	raw_printf("%s内のAUTHENTICATIONに文法エラー: %s．", configfile, opts);
	wait_synch();
}
...
*/
	raw_printf("%s内のTILESETに文法エラー: %s．", configfile, opts);
	wait_synch();
}
...
#else
	pline_The("オプション%sは%s無効指定にできません．",
		optname,
		with_parameter ? "値指定しながら" : "");
#endif
}
...
*/
			You_cant("将来のバージョンに対する警告を無効にできません．");
		else {
			Sprintf(buf,
/*JP
				"\n%s=%s Invalid reference to a future version ignored",
*/
				"\n%s=%s 将来のバージョンに対する無効な参照",
				optn, op);
			badoption(buf);
		}
		return 0;
...
*/
		pline("Slash'EM %s以前の機能に対する警告を無効にしました．",
			buf);
	}
	return 1;
}
...
#else
   while (*tmps && isspace_8(*tmps)) tmps++;
#endif
...
#else
	while (isspace_8((int)*opts)) opts++;
#endif
	op = eos(opts);
#if 0 /*JP*/
	while (--op >= opts && isspace((int)*op)) *op = '\0';
#else
	while (--op >= opts && isspace_8((int)*op)) *op = '\0';
#endif
...
*/
			pline("FRUITの指定をそんなに沢山しても実りあることにはならない．");
			return;
		    }
		}
goodfruit:
...
#else
			pline(
		  "Badoption - 巨岩のシンボル'%c'は%sのシンボルと重複しています．",
				opts[0], (clash == 1) ? "モンスター" : "警告");
#endif
		} else {
			/*
...
#if defined(VIDEOSHADES) && (!defined(NO_TERMS) || defined(MSDOS))
	/* videocolors:string */
...
#else /*JP*/
  	"-- 例えば次のようにします：$ DEFINE %s \"noautopickup,fruit:kumquat\"",
#endif /*JP*/
#endif
#if 0 /*JP*/
...
#else
		while(*newop && isspace_8(*newop)) newop++;
#endif
		if (*newop) wn = newop;
		else return 0;
