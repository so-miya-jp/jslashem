...
*/
	    if(IS_ROCK(levl[u.ux][u.uy].typ)) You("石の中にいる．");
	}
	return(TRUE);
}
...
#else
	    pline("突然，倉庫の%sが入ってきた！", g_monnam(guard));
#endif
	else
/*JP
...
*/
	    	verbalize("おい！誰がここに%sを置いて行ったんだ？", mimic_obj_name(&youmonst));
	    /* You're mimicking some object or you're hidden. */
/*JP
	    pline("Puzzled, %s turns around and leaves.", mhe(guard));
*/
	    pline("困惑しながら，%sは去っていった．", mhe(guard));
	    mongone(guard);
	    return;
	}
...
#else
	if (!strcmpi(buf, "Croesus") || !strcmpi(buf, "Kroisos") || !strcmp(buf, "クロイソス")
#endif
#ifdef TOURIST
#if 0 /*JP*/
		|| !strcmpi(buf, "Creosote")
#else
		|| !strcmpi(buf, "Creosote") || !strcmp(buf, "クレオソート")
#endif
#endif
	    ) {
...
*/
		verbalize("ほう！死の世界から戻ってきたのか？ 私が送り返してやろう！");
...
*/
		pline("%sは困惑しながら，消えた．", g_monnam(grd));
...
*/
			verbalize("金を全部置いてゆけ，ならず者め！");
			return(0);
		} else {
/*JP
			verbalize("So be it, rogue!");
*/
			verbalize("そう来るならこちらも対処させてもらうぞ，盗人め！");
			grd->mpeaceful = 0;
			return(-1);
		}
	    }
...
*/
		if(!egrd->gddone && !rn2(10)) verbalize("離れるな！");
		restfakecorr(grd);
...
#else
		"クロイソス殿へ: これは%sの%sから取り戻した金貨です．",
		jtrns_mon(mons[u.umonster].mname), plname );
#endif
