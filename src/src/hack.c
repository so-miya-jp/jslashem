...
#else
			Norep("あなたは急に%sが%sの上で成長していることに気づいた．",
			      what, surface(pos.x,pos.y));
#endif
		     }
		  }
...
moverock()
{
	...
	if (isok(rx,ry) && !IS_ROCK(levl[rx][ry].typ) &&
		...
		return (-1);
#else
	    if (revive_nasty(rx, ry, "向こう側で何かが動く気配がする．"))
		return (-1);
#endif
...
#else
		    pline("向こう側に%sがいる．", a_monnam(mtmp));
#endif
		} else {
#if 0 /*JP*/
		    You_hear("a monster behind %s.", the(xname(otmp)));
#else
		    pline("%sの向こう側に怪物の気配がする．", the(xname(otmp)));
#endif
		    map_invisible(rx, ry);
		}
		if (flags.verbose)
    ...
    xchar x, y;
{
	...
	You("start chewing %s %s.",
		...
		(boulder || IS_TREE(lev->typ)) ? "を噛み" : "に穴をあけ");
#endif
...
#else
		    pline_The("倉庫番の鉄格子はあなたの能力を受け付けない．");
#endif
		return FALSE;
	    }
...
#else
			    pline("いてっ！あなたは重い扉にぶつかった．");
#endif
			    exercise(A_DEX, FALSE);
/*JP
			} else pline("That door is closed.");
*/
			} else pline("その扉は閉まっている．");
		    }
		}
		return FALSE;
	    } else
...
*/
		if (mode == DO_MOVE) You("扉の下からにじみ出た．");
	    } else if (tunnels(youmonst.data) && !needspick(youmonst.data)) {
		/* Eat the door. */
		if (mode == DO_MOVE && still_chewing(x,y)) return FALSE;
	    } else {
...
*/
			You("扉の下からにじみ出ようとしたが，持ち物はそうはいかなかった．");
		    else if (x == ux || y == uy) {
			if (Blind || Stunned || ACURR(A_DEX) < 10 || Fumbling) {
#ifdef STEED
			    if (u.usteed) {
...
*/
			You("自分の動きをうまく制御できない．"); break;
		    case 2:
/*JP
			pline("It's hard to walk in thin air.");
*/
			pline("何もない空中を歩くのは難しい．");
			exercise(A_DEX, TRUE);
			break;
		    }
		    return;
...
#else
		    /*JP 日本語には汎用的な騎乗している動物を指し示す言葉がない・・・ */
		    Your("騎乗している%sからの反応がない！", Monnam(u.usteed));
#endif
		    nomul(0);
		    return;
		}
...
*/
		pline("%sは動こうとしない！", upstart(y_monnam(u.usteed)));
		nomul(0);
		return;
	} else
#endif
...
*/
		pline("何かを拾い上げようとしたら，あなたは丸焦げになってしまうだろう．");
		return(0);
	    }
	}
	if(!OBJ_AT(u.ux, u.uy)) {
