...
#else
		    pline("突然，%sなった．",
			See_invisible ? "あなたは透明に" : "自分自身が見えなく");
#endif
		}
		break;
...
Gloves_off(VOID_ARGS)
{
	...
	Sprintf( kbuf, "%sの死体に触れて", killer_cxname(uswapwep, TRUE));
#endif
	instapetrify(kbuf);
...
*/
			You("突然，問題になるほどの大量の水を飲み込むハメになった！");
		    	(void) drown();
		    }
		    return;
		}
...
*/
		    pline("突然，透明になっていた自分の存在を認識できるようになった！");
...
#else
	  You("装填していた%sをおさめた．", doname(uquiver));
	  setuqwep((struct obj *) 0);
#endif
