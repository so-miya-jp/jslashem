...
#else
			pline("%sは動きを止め，音楽に合わせてゆらゆらしはじめ%sた．",
			      Monnam(mtmp),
...
*/
		    pline("%sは戦闘準備に入った！", Monnam(mtmp));
		else
/*JP
		    Norep("You hear the rattle of battle gear being readied.");
*/
		    Norep("あなたは武装を準備するカチャカチャという音を聞いた．");
	    }
	    mtmp = mtmp->nmon;
	}
}
...
*/
	if (u.uevent.uheard_tune == 2 && yn("通過の調べを演奏する？") == 'y') {
	    Strcpy(buf, tune);
	} else {
/*JP
