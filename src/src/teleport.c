...
*/
		if(ynq("どこでもない場所に行きます？よろしいですか？") != 'y') return;
#if 0 /*JP*/
		You("%s in agony as your body begins to warp...",
		    is_silent(youmonst.data) ? "writhe" : "scream");
#else
	        You("身体が移動しはじめると，苦しみ%s．．．",
		    is_silent(youmonst.data) ? "で身もだえした" : "の叫び声をあげた");
#endif
...
*/
		    escape_by_flying = "羽毛のように地面に降り立った";
		} else if (Flying) {
/*JP
		    escape_by_flying = "fly down to the ground";
*/
		    escape_by_flying = "ゆっくり地面に降り立った";
		} else {
/*JP
...
*/
		    pline("自らの死への数千フィートを真っ逆さまに落ちていった．");
#if 0 /*JP:T*/
		    Sprintf(buf,
...
#else
	You("別の階への瞬間移動の罠に%s込んだ！",
	    Levitation ? (const char *)"浮いたまま入り" :
