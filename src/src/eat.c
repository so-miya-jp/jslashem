...
#define FRENCH_FRIED_TIN 11
	...
	{"", 0}
#else
static const struct { const char *txt; int nut; char after;} tintxts[] = {
	{"の揚げ物",	 60, 0x01},
	{"の漬物",	 40, 0x01},
	{"のスープ",	 20, 0x01},
	{"のピューレ",	500, 0x01},
...
#else
		You("%sを%s終えた．", food_xname(victual.piece, TRUE),
		  victual.piece->odrained ? "吸い" : "食べ");
#endif
...
*/
		Sprintf(killer_buf, "%sの肉を口にして", jtrns_mon(mons[pm].mname));
		killer_format = KILLED_BY;
		killer = killer_buf;
/*JP
		You("turn to stone.");
...
#else
		    pline("%s%s%sのはよくない気がした．",
			jtrns_mon(mons[pm].mname),
			occupation == opentin ? "の缶詰" : "",
			victual.eating ? "を食べる" : "を口にする") ;
#endif
		    HAggravate_monster |= FROMOUTSIDE;
		}
		break;
...
*/
		    pline("それを食べることは死を意味する．");
#if 0 /*JP*/
		    Sprintf(buf, "unwisely ate the body of %s",
			    mons[pm].mname);
#else
		    Sprintf(buf, "愚かにも%sの死体を食べて",
			    jtrns_mon(mons[pm].mname));
#endif
		    killer = buf;
/*JP
...
fix_petrification()
{
	...
	if (Hallucination)
	    ...
	    pline("What a pity - you just ruined a future piece of %sart!",
		  ACURR(A_CHA) > 15 ? "fine " : "");
#else
	    pline("なんてことだ！ ― 後世に残る%s芸術作品になれたのに！",
		  ACURR(A_CHA) > 15 ? "貴重な" : "");
#endif
	else
/*JP
...
*/
		...
		if(!(HTeleportation & FROMOUTSIDE)) {
			...
			You_feel(Hallucination ? "diffuse." :
			    "very jumpy.");
#else
			You(Hallucination ? "自分が拡散していくような気がした．" :
			    "とても落ち着かなくなった．");
#endif
...
#else
		    Sprintf(buf, Hallucination ?
			"突然，皮を剥かれるのが恐ろしくなってまた%sの真似をした！" :
			"こんどはまた%sの真似がしたくなった．",
			    jtrns_mon(Upolyd ? youmonst.data->mname : urace.noun));
#endif
...
*/
	    verbalize("%sのなら買ってもらうよ！", verb ? verb : "開けた");
...
#else
			pline ("まるで%sのような匂いがした．",
				jtrns_obj('S', monexplain[mons[tin.tin->corpsenm].mlet]));
#endif
		else 
#if 0 /*JP*/
...
#else
	    pline("%sのような匂いがした．", jtrns_mon(what));
#endif
...
#else /*JP*/
/*JP 
  afterフラグのついてないもの（腐った，ゆでた，乾燥，四川風，フレンチ風）は前置に，
  それ以外は後置．
 */
	
	    You("%s%s%sの缶詰をたいらげた．",
		( tintxts[r].after ) ? "" : tintxts[r].txt,
#ifdef EATEN_MEMORY
		mvitals[tin.tin->corpsenm].eaten ?
		  jtrns_mon(mons[tin.tin->corpsenm].mname) : "何か",
#else
		jtrns_mon(mons[tin.tin->corpsenm].mname),
#endif
		( tintxts[r].after ) ? tintxts[r].txt : "") ;
#endif /*JP*/
...
*/
		pline("揚げ物を食べたのであなたの%sは滑りやすくなった．",
...
#else
	    	You("%sから血を吸った．", corpse_xname(otmp,TRUE));
#endif
		otmp->odrained = 1;
	    } else {
#if 0 /*JP*/
...
#else
			pline("しかし，実のところ病気になるほどのものでもないようだが．．．");
#endif
		} else {
			char buf[BUFSZ];
			long sick_time;
...
#else
		You("%s気分が悪い．", (Sick) ? "とても" : "");
		losehp(rnd(8), "腐乱死体を食べて", KILLED_BY_AN);
#endif
	}
...
#else
		if (!retcode) pline("死体は完全に腐ってしまった．");
#endif
		if (carried(otmp)) useup(otmp);
		else useupf(otmp, 1L);
		retcode = 2;
...
#else
	Sprintf(msgbuf, "%sを%s", food_xname(otmp, TRUE), 
	  otmp->odrained ? "吸う" : "食べる");
#endif
...
struct obj *otmp;
{
	switch(otmp->otyp) {
...
#else
		    pline(Hallucination ?
			"まったりとして，それでいてしつこくない！これぞ究極のメニューだ！" :
			"この食べ物は本当に申し分ない！");
...
#else
				losehp(rnd(15), "有毒な錠剤で", KILLED_BY_AN);
	    ...
	    case MUSHROOM:
	       ...
	       otmp->cursed ? (Hallucination ? "型破りな味だ！" : "ひどくまずい！") :
...
#else
		    pline("この%sは%s", singular(otmp, xname),
		      bad_for_you ? (Hallucination ? "イケてない！" : "ひどい味だ！") :
		      (otmp->otyp == CRAM_RATION
		      || otmp->otyp == K_RATION
		      || otmp->otyp == C_RATION)
		      ? "味気ない．" :
...
*/
		pline("ヤッバダッバうまい！"); /* Flintstonesのパロディみたいです．原作よく知らず */
		exercise(A_CON, TRUE);
	}
...
struct obj *otmp;
{
	...
	if (cadaver && acidic(&mons[mnum]) && !Acid_resistance) {
		...
		Sprintf(buf, "%s rather acidic. %s",
			foodsmell, eat_it_anyway);
#else
		Sprintf(buf, "%sはかなり酸っぱそうなにおいがする．%s",
			foodsmell, eat_it_anyway);
#endif
		if (yn_function(buf,ynchars,'n')=='n') return 1;
		else return 2;
...
#else
		Sprintf(buf, "%sは穢れていて，あなたの知らないにおいがする．%s",
#endif
			foodsmell, eat_it_anyway);
		if (yn_function(buf,ynchars,'n')=='n') return 1;
		else return 2;
...
#else
		Sprintf(buf, "%sはあなたの知らないにおいがする．%s",
#endif
			foodsmell, eat_it_anyway);
		if (yn_function(buf,ynchars,'n')=='n') return 1;
		else return 2;
