...
#else
		pline(mappear, "あなたのすぐそばを囲むように");
#endif
	    else if (Displaced && (mtmp->mux != u.ux || mtmp->muy != u.uy))
#if 0 /*JP*/
		pline("%s around your displaced image!", mappear);
#else
		pline(mappear, "あなたの幻影を囲むように");
#endif
	    else
#if 0 /*JP*/
		pline("%s from nowhere!", mappear);
#else
		pline(mappear, "どこからともなく");
#endif
	}
	dmg = 0;
	break;
...
#else
	You_feel("怪物たちにあなたの存在を知られたような気がした．");
#endif
	aggravate();
	dmg = 0;
	break;
...
*/
	    Your("%sは突然激しい痛みにおそわれた！", body_part(HEAD));
	else
/*JP
	    Your("%s suddenly aches very painfully!", body_part(HEAD));
*/
	    Your("%sは突然ひどく激しい痛みにおそわれた！", body_part(HEAD));
	break;
    default:
