...
lookat(x, y, buf, monbuf)
    ...
    char *buf, *monbuf;
#else /*JP*/
/* `buf2' is simple name for search data file easily. */
...
#else
	    Strcat(buf, "，溶岩に沈んでいる");	/* [can this ever happen?] */
#endif
...
#else
	Strcpy(buf, level.flags.lethe? "泡立つ水" : "水");
	Strcpy(buf2, buf);
#endif
	break;
    default:
...
#if 1 /*JP*/
		    firstmatch_jp = jtrns_obj('S',monexplain[i]);
#endif
		    found++;
		} else {
/*JP
...
#if 1 /*JP*/
		    firstmatch_jp = jtrns_obj('S',objexplain[i]);
#endif
		    found++;
		} else {
/*JP
...
#if 1 /*JP*/
		firstmatch_jp = jtrns_obj('S', invisexplain);
#endif
		found++;
	    } else {
/*JP
...
*/
		    ...
		    firstmatch = x_str;
#if 1 /*JP*/
		    firstmatch_jp = jtrns_obj('S', x_str);
#endif
		    found++;
...
#if 1 /*JP*/
			/* ここではfirstmatchも日本語になっていることに注意 */
			firstmatch_jp = firstmatch;
#endif
			found++;
		} else {
...
#else
		firstmatch_jp = jtrns_obj('S', x_str);
		Sprintf(out_str, "%c       %s", sym, firstmatch_jp);
#endif
		firstmatch = x_str;
		found++;
	    } else {
...
*/
	...
	if (from_screen) {
		...
		/* ここではfirstmatchも日本語になっていることに注意 */
		firstmatch_jp = look_buf;
		firstmatch = look_buf2;
#endif
		if (*firstmatch) {
#if 0 /*JP*/
		    Sprintf(temp_buf, " (%s)", firstmatch);
#else
		    Sprintf(temp_buf, " (%s)", firstmatch_jp);
#endif
