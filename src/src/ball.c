...
*/
		    losehp(2, "ŒF‚Ìã©‚©‚ç”²‚¯‚æ‚¤‚Æ‘«‚ğˆø‚Á‚Ï‚Á‚Ä",
					KILLED_BY);
		}
		break;
	      }
...
*/
			...
			losehp(rnd(6), "dragged downstairs by an iron ball",
				NO_KILLER_PREFIX);
#else
			losehp(rnd(6), "“S‹…‚É‚æ‚èŠK’i‚ğ‚±‚ë‚ª‚è—‚¿‚Ä",
				KILLED_BY);
#endif
			litter();
		}
...
*/
			losehp(rnd(20), "“S‹…‚ÌÕ“Ë‚Å", KILLED_BY_AN);
...
*/
			...
			losehp(rnd(3), "dragged downstairs by an iron ball",
				NO_KILLER_PREFIX);
#else
			losehp(rnd(3), "“S‹…‚É‚æ‚èŠK’i‚ğ‚±‚ë‚ª‚è—‚¿‚Ä",
				KILLED_BY);
#endif
			exercise(A_STR, FALSE);
			litter();
