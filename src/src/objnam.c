...
#define PREFIX	80	/* (56) */
#else
/* 「透明な霞んだ呪われていない油の塗られた吸い尽くされた科学者『フランケンシュタイン』(の死体)」*/
#define PREFIX	112	/* (82) */
#endif
...
register struct obj *obj;
{
	...
	if(obj->quan != 1L)
		...
		if(objects[obj->otyp].oc_skill==-P_FIREARM)
		    Sprintf(prefix, "%ld発分の", obj->quan);
		else
...
*/
	if (wizard && is_hazy(obj)) Strcat(prefix,"霞んだ");
#endif
...
*/
	if (Hallucination ? !rn2(100) : obj->greased) Strcat(prefix, "油の塗られた");
...
*/
		    if (obj->lamplit) Strcat(bp, " (光っている)");
#  ifdef DEBUG
...
*/
		    if (obj->oarmed) Strcat(bp, " (作動している)");
#else	/* FIREARMS */
# ifdef LIGHTSABERS
...
#else
		    Strcpy(tmpbuf, (obj->oeaten > drainlevel(obj)) ? "吸いかけの" : "吸い尽くされた");
#endif
		}
		else if (obj->oeaten)
#if 0 /*JP*/
...
#if 1 /*JP*/
	Sprintf(nambuf, "%sの死体", jtrns_mon(mons[
		Hallucination ? rndmonnum() : otmp->corpsenm].mname));
	return nambuf;
#else
...
/* 特殊2バイト文字を1バイト文字へ変換 */
	...
	S("アエスキュラピスの杖", "The Staff of Aesculapius");
	S("脂の缶", "can of grease");
	...
	if(strlen(buf)>4 && !strcmp(buf + strlen(buf) - 4, "巻物"))
	  ...
	  Strcpy(pfx, "ring of ");
		...
		(strlen(buf)<4 || strcmp(buf + strlen(buf) - 4, "丸薬")) )
...
*/
	...
	/* 残っている日本語部分は怪物名かアイテム名のはずなので変換する */
	while(1)
	{
...
*/
	    pline("一瞬%sが%sの中にあるような感じがしたが，すぐに消え去った！",
		  something,
