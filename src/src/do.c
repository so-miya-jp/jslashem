...
*/
			You_hear("%sと言う音を聞いた．",lava ? "シューッ" : "パシャッ");
...
*/
		...
		if (Blind && flags.soundok)
			...
			pline("%s %s into %s pit.",
				...
				the_your[t->madeby_u]);
#else
			pline("%sは%s落し穴に転がりおちた．",
				xname(obj), set_you[t->madeby_u]);
#endif
	}
#ifdef LIGHTSABERS
...
*/
		if (cansee(x, y)) You("%sが停止するのを見た．", an(xname(obj)));
	...
	register struct obj *obj;
{
	...
	if ((obj->blessed || obj->cursed) && obj->oclass != COIN_CLASS) {
		...
		pline("%sが祭壇に触れると%s光った．",
			doname(obj),
			an(jconj_adj(hcolor(obj->blessed ? NH_AMBER : NH_BLACK))));
#endif
...
#else
		pline("腐った%sのような匂いがした．", fruitname(FALSE));
#endif
		break;
...
#else
		You("%s降りることができない．",
			!u.uswallow ? "つかまえられていて" : is_animal(u.ustuck->data) ?
			"飲み込まれていて" : "巻き込まれていて");
#endif
		return(1);
	}
...
#else
	    You("%s%s．", locomotion(youmonst.data, "跳んで"),
...
#else
		pline("新しいレベルを作るためのディスク容量が足りない！");
		You("セーブするか終了するか，あるいはプレイを続行できる．");
#endif
		return -1;
	}
#endif
...
#else
		"オワッ！まったく変わっちまってる．",
		"あなたはまがりくねった，全てが同じに見える通路にかこまれていた．",
		"ゲー！コナンおじさんの土地みたいだぞ．．．",
#endif
		0	/* no message */
	    };
...
#else
		        Sprintf(sackname, "%sの%s", mon_nam(mcarry),
  				xname(container)); 
	   		pline("%sは%sから逃れようともがいた！", Amonnam(mtmp),
	   		      sackname);
#endif
...
#else
			pline("%sは%sから逃れた！", Amonnam(mtmp), sackname);
#endif
		}
		break;
	    default:
...
*/
		You("%sからネバネバを拭った．", body_part(FACE));
		u.ucreamed = 0;
		Blinded = 1;
