...
*/
		if(Blind || !flags.verbose) pline("何かがあなたをかすめた．");
/*JP
		else You("are almost hit by %s.", onm);
*/
		else pline("もう少しで%sが命中するところだった！",onm);
		return(0);
	} else {
/*JP
...
*/
		else pline("%sがあなたに命中した%s", onm, exclam(dam));
...
*/
	    else if (verbose) pline("%sに命中した%s", Monnam(mtmp), exclam(damage));
...
*/
	        if (vis) pline("銀が%sの肉体を焦がした！",
				s_suffix(mon_nam(mtmp)));
/*JP
		else if (verbose) pline("Its flesh is seared!");
*/
		else if (verbose) pline("何者かの肉体が焦げてゆく！");
	    }
...
*/
		    else if (verbose) pline("何者かは焼かれた！");
		}
	    }
	    mtmp->mhp -= damage;
	    if (mtmp->mhp < 1) {
...
*/
		    pline("%sは%sによって目を潰された．", Monnam(mtmp), the(xname(otmp)));
		mtmp->mcansee = 0;
...
*/
			    You("思惑があるのではないかと想像しながら%sの贈り物を受けとった．",
				s_suffix(mon_nam(mon)));
				...
				"You catch:");
#else
				"あなたは%sを受け止めたが，落した．", xname(singleobj),
				"をつかまえた．");
#endif
			}
			break;
...
*/
			    if(!Blind) pline("ウェー．あなたはクリームまみれになった．");
#if 0 /*JP*/
			    else pline("There's %s sticky all over your %s.",
				       something,
				       body_part(FACE));
#else
			    else pline("%sべとべとするものがあなたの%sを覆った．",
				       something,
				       body_part(FACE));
#endif
			    ...
			    else Your("%sに鋭い痛みが走った．", makeplural(body_part(EYE)));
#endif
			}
		    }
