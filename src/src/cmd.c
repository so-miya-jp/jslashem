...
#define MAX_EXT_CMD 50		/* Change if we ever have > 40 ext cmds */
/*
...
*/
						You("%sを盗んだ．", doname(gold));
					} else {
/*JP
            You("grab %s, but find no room in your knapsack.", doname(gold));
*/
            You("%sを掴んだが，あなたのナップザックには入れる隙間がない．", doname(gold));
			    	dropy(gold);
					}
				}
				else
...
STATIC_PTR int
wiz_toggle_invulnerability(VOID_ARGS)
{
	if (wizard) {
...
int final;	/* 0 => still in progress; 1 => over, survived; 2 => dead */
{
	...
	else if (u.ualign.record > 13)	you_have("誠実な信仰心");
	else if (u.ualign.record > 8)	you_have("忠実な信仰心");
	else if (u.ualign.record > 3)	you_have("大げさな信仰心");
	else if (u.ualign.record == 3)	you_have("通常の信仰心");
	else if (u.ualign.record > 0)	you_have("迷い多き信仰心");
	else if (u.ualign.record == 0)	you_have("名ばかりの信仰心");
	else if (u.ualign.record >= -3)	you_are_ing("信仰に迷いをもって");
	...
	if (Drain_resistance) you_have("生命力吸収への耐性");
...
#else
	if (Stoned) enl_msg(You_, "なりつつある", "なった", "石に");
	if (Slimed) enl_msg(You_, "なりつつある", "なった", "スライムに");
	...
	if (Warn_of_mon && flags.warntype) {
	    ...
	    if (warntype) {
		if (nth) strcat(buf, "及び");
		strcat(buf, something) ;
	    }
	    strcat(buf, "の存在を感じる能力") ;
	    you_have(buf);
	}
#endif
...
#else
	if (u.usteed && (final < 2 || strcmp(killer, "騎乗中の事故で"))) {
...
#ifdef WIZARD
	    if (wizard)
		Sprintf(buf, "%sに飲み込まれ(%u)て", a_monnam(u.ustuck), u.uswldtim);
	    else
#endif
...
*/
	if (Slow_digestion) enl_msg("あなたは食物の消化が遅", "い", "かった", "");
...
#else
	if (Unchanging) enl_msg(You_, "できない", "できなかった",
				"今の姿から変化することが");
	if (Fast) you_have(Very_fast ? "とても素早く行動する能力" :
			   "素早く行動する能力");
	if (Reflecting) you_have("反射能力");
	if (Free_action) you_have("拘束されない能力");
	if (Fixed_abil) you_are_ing("能力が固定されて");
	if (uamul && uamul->otyp == AMULET_VERSUS_STONE)
		you_are_ing("石化から保護されて");
	if (Lifesaved)
		enl_msg("あなたの生命は保存されて", iru, ita, "");
	if (u.twoweap) you_are("二刀流");
	if (u.twoweap) {
	    if (uwep && uswapwep)
		Sprintf(buf, "二刀流で戦闘して");
	    else if (uwep || uswapwep)
		Sprintf(buf, "武器と%s%sで戦闘して",
			uwep ? "左" : "右", body_part(HAND));
	    else
		Sprintf(buf, "両%sで戦闘して",
			makeplural(body_part(HAND)));
	    you_are_ing(buf);
	}
#endif
	...
	if (u.uhealbonus)
	{
		Sprintf(buf, "%s健康的", u.uhealbonus > 0 ? "極めて" : "不");
#ifdef WIZARD
	    if (wizard) Sprintf(eos(buf), " (%d)", u.uhealbonus);
#endif
		you_are(buf);
...
#ifdef WIZARD
	 else if (wizard) enl_msg("あなたの健康度はゼロ", "である", "だった", "");
#endif
#endif /*JP*/
...
#else /*JP*/
	{'O', TRUE, doset, "オプション"},
	{'r', TRUE, doredraw, "再描画する"},
	{'x', TRUE, enter_explore_mode, "探索モードに入る"},
#ifdef SHELL
	{'!', TRUE, dosh, "Shellを呼び出す"},
#endif
	{'S', TRUE, dosave, "保存して終了"},
	{'q', TRUE, done2, "保存せずに終了 [M-q]"},
#endif /*JP*/
	{0,0,0,0},
};
...
#else /*JP*/
	{(char)0, TRUE, (void *)0, "持ち物表示"},
	{'i', TRUE, ddoinv, "持ち物一覧"},
	{'I', TRUE, dotypeinv, "種類別の持ち物一覧"},
	{'*', TRUE, doprinuse, "使用できるアイテム"},
	{(char)0, TRUE, (void *)0, "装備する"},
	{'w', FALSE, dowield, "武器の装備"},
	{'W', FALSE, dowear, "防具の装備"},
	{'Q', FALSE, dowieldquiver, "射出武器の準備 (矢筒に)"},
	{'T', FALSE, dotakeoff, "防具を脱ぐ"},
	{(char)0, TRUE, (void *)0, "使用する"},
	{'a', FALSE, doapply, "アイテムを使用する"},
	{'d', FALSE, dodip, "アイテムを漬ける [M-d]"},
	{'E', FALSE, doengrave, "床に文字を書く"},
	{'f', FALSE, dofire, "射撃武器を射出する"},
	{'e', TRUE, doinvoke, "聖器を発動させる [M-i]"},
	{'t', FALSE, dothrow, "投げる"},
	{(char)0, TRUE, (void *)0, "落す"},
	{'d', FALSE, dodrop, "アイテムを落す"},
	{'D', FALSE, doddrop, "複数選択で落す"},
#endif /*JP*/
	{0,0,0,0}
};
...
#else /*JP*/
	{'c', FALSE, doclose, "扉を閉める"},
	{'e', FALSE, doeat, "食事をとる"},
	{'f', FALSE, doforce, "錠をこじ開ける [M-f]"},
	{'l', FALSE, doloot, "ものを調べる"},
	{'o', FALSE, doopen, "扉を開ける"},
	{'q', TRUE, dodrink, "水薬を飲む"},
	{'r', FALSE, doread, "読む"},
	{'u', FALSE, dountrap, "解除する [M-u]"}, 
	{'z', FALSE, dozap, "杖を振る"},
	{'Z', TRUE, docast, "呪文を詠唱"},
#endif /*JP*/
	{0,0,0,0}
...
#else /*JP*/
	{'b', FALSE, playersteal, "怪物から盗む [M-b]"},
	{'c', TRUE, dotalk, "怪物と話す [M-c]"},
	{'d', FALSE, dokick, "蹴る"}, 
/*	{'e', FALSE, specialpower, "職業の特殊能力を使う [M-e]"},*/
	{'e', TRUE, enhance_weapon_skill, "武器スキル [M-k]"},
	{'m', TRUE, domonability, "種族の能力を使用する [M-m]"},
	{'o', FALSE, dosacrifice, "捧げる [M-o]"},
	{'p', FALSE, dopay, "店主に支払う[M-p]"},
	{'s', FALSE, dosit, "座る [M-s]"},
	{'t', TRUE, dotele, "瞬間移動する [C-t]"},
/*	{'T', TRUE, doturn, "不死者を還す [M-t]"},*/
	{'T', TRUE, dotech, "特殊能力を使う [M-t]"},
	{'x', TRUE, doattributes, "属性を見る [C-x]"},
	{'y', TRUE, polyatwill, "変身する [M-y]"},
#endif /*JP*/
	{0,0,0,0}
};
...
#endif
#else /*JP*/
	{'c', TRUE, wiz_gain_ac, "ACの上昇"},
#ifdef DISPLAY_LAYERS
	{'d', TRUE, wiz_show_display, "表示レイヤーの詳細"},
#endif
	{'e', TRUE, wiz_detect, "扉と罠の表示"},
	{'f', TRUE, wiz_map, "魔法の地図の実行"},
	{'g', TRUE, wiz_genesis, "怪物生成"},
	{'i', TRUE, wiz_identify, "所持品の鑑定"},
	{'j', TRUE, wiz_gain_level, "経験レベルの上昇"},
	{'n', TRUE, wiz_toggle_invulnerability, "無敵化トグル"},
	{'o', TRUE, wiz_where, "特殊レベルの一覧"},
	{'v', TRUE, wiz_level_tele, "別の階へ瞬間移動"},
	{'w', TRUE, wiz_wish,  "願いを叶える"},
	{'L', TRUE, wiz_light_sources, "移動光源を見る"},
	{'M', TRUE, wiz_show_stats, "メモリー状態を見る"},
	{'S', TRUE, wiz_show_seenv, "show seen vectors"},
	{'T', TRUE, wiz_timeout_queue, "タイムアウトキューの表示"},
	{'V', TRUE, wiz_show_vision, "視界配列の参照"},
	{'W', TRUE, wiz_show_wmodes, "壁透視モード"},
#ifdef DEBUG
	{'&', TRUE, wiz_debug_cmd, "wizard debug command"},
#endif
#endif /*JP*/
...
#else
	{'?', TRUE, dohelp, "ヘルプ一覧"},
	{'v', TRUE, doextversion, "バージョン"},
	{'/', TRUE, dowhatis, "画面上の文字の意味を調べる" },
	{'&', TRUE, dowhatdoes, "キーの動作を調べる"},
#endif
	{0,0,0,0,0},
};
...
#else
	{'X', TRUE, dovspell, "記憶している呪文"},                  /* Mike Stephenson */
	{'d', TRUE, dodiscovered, "発見済みアイテム [\\]"},             /* Robert Viduya */
	{'C', TRUE, do_mname, "画面上の怪物の名前"},
#endif
	{0,0,0,0},
};
...
static struct menu_list main_menustruct[] = {
	...
	{0,0,0},
#else /*JP*/
	{"ゲーム", "メインメニュー", game_menu},
	{"所持品", "メインメニュー", inv_menu},
	{"行動", "メインメニュー", action_menu},
	{"プレイヤー", "メインメニュー", player_menu},
	{"発見物", "メインメニュー", discover_menu},
#ifdef WIZARD
	{"Wizard", "メインメニュー", wizard_menu},
#endif
	{"ヘルプ", "メインメニュー", help_menu},
	{"メインメニュー",(char *)0, main_menu},
	{0,0,0},
#endif /*JP*/
};
...
STATIC_PTR int
domenusystem(VOID_ARGS)  /* WAC add helpful menus ;B */
{
#if 0 /*JP*/
        return (makemenu("Main Menu", main_menustruct));
#else
        return (makemenu("メインメニュー", main_menustruct));
#endif
}
...
#else
	    Sprintf(buf, "純潔の誓約を%ld回破って", u.uconduct.celibacy);
	    you_are_ing(buf);
#endif
	}
#endif
...
#else /*JP*/
	{"2weapon", "両手持ちの切り替え", dotwoweapon, FALSE},
	{"adjust", "持ち物一覧の調整", doorganize, TRUE},
	{"borrow", "モンスターから盗む", playersteal, FALSE},  /* jla */
	{"chat", "誰かと話す", dotalk, TRUE},	/* converse? */
	{"conduct", "こだわりの挑戦一覧", doconduct, TRUE},
	{"dip", "何かに物を浸す", dodip, FALSE},
	{"enhance", "武器熟練度を高める", enhance_weapon_skill, TRUE},
#if 0
	{"ethics", "自発的挑戦一覧", doethics, TRUE},
#endif
	{"explore", "探索モードに入る", enter_explore_mode, TRUE},
	{"force", "錠をこじあける", doforce, FALSE},
	{"invoke", "物の特別な力を使う", doinvoke, TRUE},
	{"jump", "指定地点に跳躍する", dojump, FALSE},
	{"loot", "床の上の箱を開ける", doloot, TRUE},
	{"monster", "怪物の特別能力を使う", domonability, TRUE},
	{"name", "アイテムに固有名もしくは識別名をつける", ddocall, TRUE},
	{"offer", "神に供物を捧げる", dosacrifice, FALSE},
	{"pray", "助けを乞うために神に祈る", dopray, TRUE},
	{"quit", "セーブしないで終了", done2, TRUE},
#ifdef STEED
	{"ride", "怪物に乗る(または降りる)", doride, FALSE},
#endif
	{"rub", "ランプか石をこする", dorub, FALSE},
#ifdef SHOUT
	{"shout", "大声で叫ぶ", doyell, TRUE}, /* jrn */
#endif
	{"sit", "座る", dosit, FALSE},
	{"technique", "技を行使する", dotech, TRUE},
	{"turn", "アンデッドを祓う", doturn, TRUE},
	{"twoweapon", "両手持ちの切り替え", dotwoweapon, FALSE},
	{"untrap", "罠をはずす", dountrap, FALSE},
	{"vanquished", "倒したモンスター一覧", dolistvanq, TRUE},
	{"version", "コンパイル時のオプションを表示する",
		doextversion, TRUE},
	{"wipe", "顔を拭う", dowipe, FALSE},
	{"youpoly", "意識的に変身する", polyatwill, FALSE},  /* jla */
...
#else /*JP*/
#ifdef DISPLAY_LAYERS
	{"display", "表示レイヤーの詳細", wiz_show_display, TRUE},
#endif
	{"levelchange", "経験レベルを変える", wiz_level_change, TRUE},
	{"light sources", "移動光源を見る", wiz_light_sources, TRUE},
#ifdef DEBUG_MIGRATING_MONS
	{"migratemons", "指定匹数のランダムな怪物を召喚する", wiz_migrate_mons, TRUE},
#endif
	{"monpolycontrol", "怪物への変化を制御する", wiz_mon_polycontrol, TRUE},
...
#else
	    putstr(win, 0, "有効な方向キー(number_pad オプション有効時):");
#endif
	    putstr(win, 0, "             8   ");
	    putstr(win, 0, "             |   ");
	    putstr(win, 0, "          4- . -6");
