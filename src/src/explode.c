...
#else
		case 4: str = (olet == WAND_CLASS) ? "死の空間" :
							"分解空間";
#endif
			adtyp = AD_DISN;
			break;
...
#else
		You("%s爆発音を聞いた．", is_pool(x, y) ? "くぐもった" : "");
#endif
	}
			...
			if (is_animal(u.ustuck->data))
				...
				if (!silent) pline("%s gets %s!",
				      ...
				      (adtyp == AD_ACID) ? "an upset stomach" :
				       "fried");
#else
				if (!silent) pline("%sは%s！",
				      Monnam(u.ustuck),
				      (adtyp == AD_FIRE) ? "胸焼けをおこした" :
				      (adtyp == AD_COLD) ? "芯から冷えた" :
				      (adtyp == AD_DISN) ? ((olet == WAND_CLASS) ?
				       "純粋なエネルギーに曝された" : "穴をあけられた") :
				      (adtyp == AD_ELEC) ? "感電した" :
				      (adtyp == AD_DRST) ? "毒に冒された" :
				      (adtyp == AD_ACID) ? "胃がムカムカした" :
				       "パリパリになった");
#endif
			else
#if 0 /*JP*/
				if (!silent) pline("%s gets slightly %s!",
				      ...
				      (adtyp == AD_ACID) ? "burned" :
				       "fried");
#else
				if (!silent) pline("%sは少しだけ%s！",
				      Monnam(u.ustuck),
				      (adtyp == AD_FIRE) ? "焦げた" :
				      (adtyp == AD_COLD) ? "冷やされた" :
				      (adtyp == AD_DISN) ? ((olet == WAND_CLASS) ?
				       "純粋なエネルギーに覆われた" : "穴をあけられた") :
				      (adtyp == AD_ELEC) ? "電撃をくらった" :
				      (adtyp == AD_DRST) ? "毒をくらった" :
				      (adtyp == AD_ACID) ? "酸で焼かれた" :
				       "パリパリになった");
#endif
...
*/
		...
		if (u.uhp <= 0 || (Upolyd && u.mh <= 0)) {
			...
			if (olet == MON_EXPLODE) {
			    ...
			    Sprintf(killer_buf, "自分の起こした%sに巻き込まれて",
				    str);
#endif
...
*/
    if (shop_damage) pay_for_damage("傷つける", FALSE);
}
