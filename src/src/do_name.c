...
*/
    Sprintf(sbuf, "[%s]でカーソルを%sへ移動できる．",
	    iflags.num_pad ? "2468" : "hjkl", goal);
    putstr(tmpwin, 0, sbuf);
/*JP
    putstr(tmpwin, 0, "Use [HJKL] to move the cursor 8 units at a time.");
*/
    putstr(tmpwin, 0, "[HJKL]でカーソルを一度に8歩移動できる．");
/*JP
...
#else
    Sprintf(sbuf, "目的地にカーソルを移動したら，[.]%sで決定．",
            doing_what_is ? "[,][;][:]" : "");
#endif
    putstr(tmpwin, 0, sbuf);
    if (!force)
...
*/
	putstr(tmpwin, 0, "スペースまたはエスケープで中断．");
    putstr(tmpwin, 0, "");
