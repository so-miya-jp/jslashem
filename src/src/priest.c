...
#else /*JP*/
			if (mon->data == &mons[PM_HIGH_PRIEST]) {
				Strcat(pname, Hallucination ? "無能大臣"
					: mon->female ? "女教皇" : "法王");
			} else {
				Strcat(pname, Hallucination ? "無能官僚"
					: mon->female ? "尼僧" : "僧侶");
			}
#endif /*JP*/
		}
...
*/
		    pline("巨大な幽霊があなたのすぐそばに現れた！");
/*JP
		    else You("sense a presence close by!");
*/
		    else You("すぐそばに何かがいるのを感じた！");
		    mtmp->mpeaceful = 0;
		    set_malign(mtmp);
		    if(flags.verbose)
/*JP
...
#else
		pline("%sは瞑想から我に返った！", Monnam(priest));
#endif
...
*/
	    pline("%sの声が響いた：「我が下僕に歯向かう覚悟はできたか！」",
