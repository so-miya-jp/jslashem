...
#else
	    char *umname = jtrns_mon_gen(mons[u.umonnum].mname, flags.female) ;
	    if ( strlen(umname) > SIZE(mbot) - 1 && is_kanji1(umname, SIZE(mbot) - 1) ) {
		(void) strncpy(mbot, umname, SIZE(mbot) - 2);
		mbot[SIZE(mbot) - 2] = '_' ;
	    } else {
		(void) strncpy(mbot, umname, SIZE(mbot) - 1) ;
	    }
#endif
	mbot[SIZE(mbot) - 1] = 0;
		while(mbot[k] != 0) {
...
#else
	Sprintf(nb = eos(nb), "ã≠:%s ", botl_strength());
	Sprintf(nb = eos(nb),
...
#else
			Is_astralevel(&u.uz) ? "ìVè„äE " : "ç≈èIééó˚ ");
#endif
	else {
		if (verbose)
#if 0 /*JP*/
...
#else
	Sprintf(nb = eos(nb), "ëÃ:%d(%d) ñÇ:%d(%d) äZ:%-2d",
#endif
...
#else
		Sprintf(nb = eos(nb), " â◊èd:%ld/%ld", (long)(inv_weight()+weight_cap()),
#endif
				(long)weight_cap());
#endif
...
#else
	if(Levitation)
		Sprintf(nb = eos(nb), bot2_abbrev >= 2 ? " ïÇ" : " ïÇóV");
	if(Confusion)
		Sprintf(nb = eos(nb), bot2_abbrev >= 2 ? " óê" : " ç¨óê");
	if(Sick) {
		if (u.usick_type & SICK_VOMITABLE)
			   Sprintf(nb = eos(nb),
			     bot2_abbrev >= 2 ? " ì≈" : " êHì≈");
		if (u.usick_type & SICK_NONVOMITABLE)
			   Sprintf(nb = eos(nb),
			     bot2_abbrev >= 2 ? " ïa" : " ïaãC");
	}

	if(Blind)
		Sprintf(nb = eos(nb), bot2_abbrev >= 2 ? " ñ”" : " ñ”ñ⁄");
	if(Stunned)
		Sprintf(nb = eos(nb), bot2_abbrev >= 2 ? " ·ø" : " ·øùÚ");
	if(Hallucination)
		Sprintf(nb = eos(nb), bot2_abbrev >= 2 ? " å∂" : " å∂äo");
	if(Slimed)
		Sprintf(nb = eos(nb), bot2_abbrev >= 2 ? " ón" : " ónâ");
	if(u.ustuck && !u.uswallow && !sticks(youmonst.data))
		Sprintf(nb = eos(nb), bot2_abbrev >= 2 ? " ïﬂ" : " ïﬂîõ");
	if(cap > UNENCUMBERED)
		Sprintf(nb = eos(nb), " %s",
		  bot2_abbrev >= 2 ? enc_abbrev_stat[cap] : enc_stat[cap]);
#endif
