...
*/
	    pline("%sにはもう鞍が取りつけられている．", Monnam(mtmp));
	    return 1;
	}
	ptr = mtmp->data;
...
#else
	    Your("%sは騎乗できる状態ではない．", makeplural(body_part(LEG)));
#endif
#ifdef WIZARD
...
*/
		You("鞍にまたがるために%sを上げることができない．", body_part(LEG));
	    else
/*JP
...
#else
	    pline("%sは%sに捕まっているので乗れない．", mon_nam(mtmp),
...
*/
		losehp(rn1(10,10), "騎乗中の事故で", KILLED_BY_AN);
...
*/
			pline("%sは溶岩に引きずり込まれた！", Monnam(mtmp));
			if (!likes_lava(mdat)) {
			    killed(mtmp);
			    adjalign(-1);
			}
