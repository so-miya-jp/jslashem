...
*/
	...
	if (t_at(u.ux, u.uy) && u.utraptype == TT_PIT) {
	    if (could_pass_walls && !Levitation) {	/* player forms cannot pass walls */
		u.utrap = rn1(6,2);
	    }
	}
...
*/
	if (!uarmg) selftouch("石化への耐性がなくなって，あなたは");
...
*/
			You("声が響きわたった: 「何故に滅ぼすべき者の姿をとろうとするのか！」");
			u.ualign.record -= 5;
...
#else
			You("%sに変化しはじめた．しかし声が響きわたった:",
			    jtrns_mon_gen(mons[mntmp].mname, flags.female));
			pline("「そなたをそのような姿にさせはせぬ！」");
#endif
		 }
		 return(0);
	}
...
*/
			Sprintf(qbuf, "本当に%sを睨みつけますか？", mon_nam(mtmp));
...
*/
	    pline("%sはあなたの睨みに気付いていないようだ．", Monnam(mtmp));
	    return (1);
	} else {
...
*/
		pline("くもの巣は%sの中に解けて消えた．", mon_nam(u.ustuck));
		return(0);
	}
	if (u.utrap) {
/*JP
...
#else
			    pline("%sは炎に焼かれなかった！", mon_nam(mtmp));
#endif
			    dmg = 0;
			}
