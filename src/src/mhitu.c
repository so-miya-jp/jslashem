...
#else
		pline("%sの攻撃は黄金色のかすみによってさえぎられた．", Monnam(mtmp));
#endif
	    else
#if 0 /*JP*/
		Your("%s %s%s %s attack.", 
...
*/
		pline("%sはあなたに触れようとして空を切った！", Monnam(mtmp));
	    else
		switch(rn2(3)) {
#if 0 /*JP*/
...
#else
		pline("%sは水の反射に惑わされ，はずした！",Monnam(mtmp));
#endif
...
*/
			  pline("%sはもう少しで降ってきた穴喰い(あなた)に当たるところだった！",
								Monnam(mtmp));
		    }
		} else {
		    if (!youseeit)
...
*/
			pline("何者かがあなたが隠れているところに踏み込もうとしたを移動しようとした．");
		    else {
	...
	if (youmonst.data->mlet == S_MIMIC && youmonst.m_ap_type &&
		...
		if (!youseeit) pline("It gets stuck on you.");
			   ...
			   m_monnam(mtmp), youmonst.data->mname, plname);
#else
		if (!youseeit) pline("何者かがあなたに引っかかった．");
...
#else
				    pline("%sはあなたの心臓を杭で突き刺した．",
					    Monnam(mtmp));
				    killer = "心臓を木の杭で突き刺されて";
#endif
...
#else
			  pline("%sによってあなた%sは%s！",
				(Blind ? "それ" : Yname2(otmp)),
			  	(water ? "の一部" : ""),
				(water ? "蒸発してしまった" : "燃やされた"));
#endif
...
#else
			if (Blind) You("容赦なくくすぐられた！");
			else You("容赦なく%sにくすぐられた！", mon_nam(mtmp));
#endif
			nomovemsg = 0;	/* default: "you can move again" */
			nomul(-rnd(10));
			exercise(A_DEX, FALSE);
...
#else
		    pline("%sはあなたの%s%sに近寄ろうとした！", Monnam(mtmp),
			  sidestr, body_part(LEG));
#endif
		    dmg = 0;
		  } else if (mtmp->mcan) {
...
#else
			    pline("%sはあなたの%s%sの覆われていない部分をちくりと刺した！",
				Monnam(mtmp), sidestr, body_part(LEG));
#endif
			else if (!rn2(5))
/*JP
...
*/
			    You_hear("%sがシューッと唸る声を聞いた！", mon_nam(mtmp));
			if(!rn2(10) ||
...
#else
				Sprintf(dkiller_buf, "%sの攻撃で", jtrns_mon(mtmp->data->mname));
...
#else
			pline("%sはあなたを%sしようとしたが，あなたは%s．",
			    Adjmonnam(mtmp, "地味な"),
			    do_charm ? "魅了" : "誘惑",
			    do_charm ? "影響を受けない" : "興味を持てない");
#endif
		    }
		    if(rn2(3)) {
			if (!tele_restrict(mtmp)) (void) rloc(mtmp, FALSE);
		  ...
		  default:
			...
			if (is_animal(mtmp->data) && *buf) {
				...
				pline("%s tries to %s away with %s.",
				      ...
				      buf);
#else
				pline("%sは%sを持って%s逃げ出した．",
				      Monnam(mtmp),
				      buf, locomotion(mtmp->data, ""));
#endif
			}
...
*/
		      verbalize("先生，ご協力いただかないとお助けできません．");
			dmg = 0;
...
*/
		You("どうも%s好みの味じゃなかったようだ．", mon_nam(mtmp));
	    expels(mtmp, mtmp->data, FALSE);
	}
	return(1);
}
...
*/
		        if (flags.verbose) You("爆発に巻き込まれた！");
		    }
		...
		if(!mtmp->mcan && canseemon(mtmp) &&
...
#else
			if (!Blind) pline("%sはあなたの幻影を睨みつけた！",Monnam(mtmp));
#endif
			    break;
		    }
...
#else
			if (!Blind) pline("%sはあなたのいるあたりを睨みつけたがはずれた！",Monnam(mtmp));
#endif
			break;
		    }
#if 0 /*JP*/
...
#else
		    if (!Blind) pline("%sはあなたを真っ向から睨みつけた！",Monnam(mtmp));
#endif
			...
			if(!Blind) {
		    	    ...
		    	    (void) ureflects("%s gaze is reflected by your %s.",
		    			s_suffix(Monnam(mtmp)));
#else
		    	    (void) ureflects("%sの睨みはあなたの%sによって反射した．",
		    			s_suffix(Monnam(mtmp)));
#endif
#if 0 /*JP*/
			...
			else You("%sによって突然眠り込んでしまった！",mon_nam(mtmp));
#endif
		    }
		}
		break;
...
#else
	                pline("%sはあなたの目を凝視した．．．", Monnam(mtmp));
	                poisoned("睨み", A_STR, jtrns_mon(mtmp->data->mname), 30);
#endif
	        }
	        break;
	    case AD_PLYS:
...
#else
	                pline("%sは瞬きしながらあなたを凝視した！", Monnam(mtmp));
#endif
	                if(flags.verbose)
#if 0 /*JP*/
	                        Your("position suddenly seems very uncertain!");
...
#else
		        if (vis) pline("どういうわけか，%sの武器は影響を受けない．",
#endif
...
#endif
#else /*JP*/
	if(!uarmc) mayberem(uarm, "スーツ");
	mayberem(uarmf, "ブーツ");
	if(!uwep || !welded(uwep)) mayberem(uarmg, "小手");
	if (uarms) mayberem(uarms, "盾");
	mayberem(uarmh, "兜");
#ifdef TOURIST
	if(!uarmc && !uarm) mayberem(uarmu, "シャツ");
#endif
#endif /*JP*/
...
const char *str;
{
	...
	if (rn2(20) < ACURR(A_CHA)) {
		...
		Sprintf(qbuf,"「%sを取っていい，%s？」",
			str,
			(!rn2(2) ? "あなた" : flags.female ? "ハニー" : "ダーリン" ));
#endif
		if (yn(qbuf) == 'n') return;
	} else {
		char hairbuf[BUFSZ];
		...
		if (u.umonnum == PM_FLOATING_EYE) {
			...
			if (Blind)
			    ...
			    pline("As a blind %s, you cannot defend yourself.",
							youmonst.data->mname);
#else
			    pline("盲目の%sであるあなたは，にらみで防御することができない．",
