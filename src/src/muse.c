...
*/
	    static const char *empty = "薬は空っぽだったようだ．";
...
*/
			pline("%sが瓶を開けると，中から巨大な%sが出てきた！",
			       mon_nam(mon),
/*JP
			       Hallucination ? rndmonnam() : (const char *)"ghost");
*/
			       jtrns_mon(Hallucination ? rndmonnam() : (const char *)"ghost"));
/*JP
...
*/
		pline("%sは%sを振った！", Monnam(mtmp), an(xname(otmp)));
		stop_occupation();
	}
}
...
*/
	    pline("混乱しているので，%sは魔法の言葉を間違って唱えた．．．",
...
*/
		if (vismon) pline("%sははしごをのぼって逃げた！", Monnam(mtmp));
		migrate_to_level(mtmp, ledger_no(&u.uz) - 1,
...
*/
		if (vismon) pline("%sははしごを降りて逃げた！", Monnam(mtmp));
		migrate_to_level(mtmp, ledger_no(&u.uz) + 1,
...
*/
			    pline("%sは階段をのぼって逃げた！", Monnam(mtmp));
			if(Inhell) {
	...
	case MUSE_TELEPORT_TRAP:
		...
		if (vis) {
			...
			pline("%s %s onto a teleport trap!", Monnam(mtmp),
				makeplural(locomotion(mtmp->data, "jump")));
#else
			pline("%sは瞬間移動の罠に%s込んだ！", Monnam(mtmp),
...
#else
		if (vismon) pline("%sは気分がよくなってきたようだ．", Monnam(mtmp));
#endif
...
*/
		if (vismon) pline("%sは気分がよさそうだ．", Monnam(mtmp));
...
*/
		if (vismon) pline("%sはとても気分がよさそうだ．", Monnam(mtmp));
...
*/
		    if (vismon) pline("%sは生命に満ちているように見える．", Monnam(mtmp));
		}
		else if (vismon)
/*JP
		    pline("%s discards the congealed blood in disgust.", Monnam(mtmp));
*/
		    pline("%sは凝固した血液をいまいましそうに投げ捨てた．", Monnam(mtmp));
...
*/
					pline("%sは突然弱くなったように見えた！", Monnam(mtmp));
				}
			}
		}
		if (cansee(mtmp->mx, mtmp->my) && zap_oseen)
...
*/
		    pline_The("何もない空間の%sの上からゴロゴロと音が聞こえてきた！",
			ceiling(mtmp->mx, mtmp->my));
		    if (mtmp->minvis)
...
*/
		(void) mon_poly(mtmp, FALSE, "%sは突然変化した！");
#endif
...
*/
		    const char *The_whip = vismon ? "鞭" : "鞭";
		    int where_to = rn2(4);
		    struct obj *obj = uwep;
		    const char *hand;
...
*/
			    pline("%sは%sをあなたの%sから引き寄せた！", Monnam(mtmp),
				  the_weapon, hand);
...
*/
			    pline("%sは%sを%sに叩き落した！", Monnam(mtmp),
...
*/
	You_hear("%sという音を聞いた．", (obj->otyp == POT_ACID) ? "ゴクン" : "クチャクチャ");
    m_useup(mon, obj);
...
*/
    pline("なんてこった ─ %sは後世に残る芸術作品になれたのに！",
	    mon_nam(mon));
	else
/*JP
	    pline("%s seems limber!", Monnam(mon));
*/
	    pline("%sの身体は柔らかさを取り戻したようだ！", Monnam(mon));
    }
...
*/
	    pline("%sは落ち着きを取り戻したようだ．", Monnam(mon));
    }
