...
**	For SLASH'EM 0.0.7 Copyright (c) Masaki Miyaso, Shiganai Sakusha, 2004-2005
...
int skill;
{
	...
	You("%sスキルを高める%s．",
		...
		(skill <= P_LAST_H_TO_H) ? "戦いの" : "",
	    !P_RESTRICTED(skill) ? "自信が湧いてきた" : "確信が持てた");
#endif
}
...
#else
			    pline("%sの%sは%sから離れない！",
				  Monnam(mon), xname(mw_tmp), mon_hand);
#endif
			}
			mw_tmp->bknown = 1;
		    }
...
*/
		if (canseemon(mon)) pline("%sの%sの光の刃はまたたいて消えた．", 
		...
		if (canseemon(mon)) {
			...
			pline("%s ignites %s.", Monnam(mon),
				an(xname(obj)));
#else
			pline("%sは%sの光の刃を発現させた．", Monnam(mon),
				xname(obj));
#endif
		}	    	
		begin_burn(obj, FALSE);
...
#else
			pline("%sは%sの柄頭側の光の刃も発現させた．", Monnam(mon),
				xname(obj));
#endif
		    	obj->altmode = TRUE;
		    	return;
...
char *buf;
{
	...
	case P_GRAND_MASTER: 
            if (skill <= P_LAST_WEAPON)
	    	    ...
	    	    ptr = "グランドマスター"; 
	    else ptr = "破天荒";	/* unprecedented-"前例のない,空前の,無比の,新しい,新奇な."*/
	    break;
...
*/
	    || (wizard && (yn("そのスキルは通常なら頭打ちですが，更に修行しますか？") == 'y'))
#endif
	    ) {
		if (uwep)    
/*JP
...
*/
		    You("%sを振り，真剣に修行を開始した．",doname(uwep));
		else
#if 0 /*JP*/
...
#else
	    	/*日本語では「腕を振り」の方が良いか? また小手をつけているかどうかは省略*/
		    You("%sを振り，真剣に修行を開始した．",
