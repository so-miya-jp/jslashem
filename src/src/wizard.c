...
*/
 "魔除けの所有を誰かに知られてしまうという鳥肌物の感覚をおぼえた．"
		    );
		return;
	    }
}
...
*/
		    ...
		    verbalize("%s %s!",
			  ...
			  random_insult[rn2(SIZE(random_insult))]);
#else
		    verbalize(random_malediction[rn2(SIZE(random_malediction))],
			  random_insult[rn2(SIZE(random_insult))]);
#endif
