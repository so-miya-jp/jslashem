...
static const char *random_mesg[] = {
	...
	"ユー・ガット・メール！", /* AOL */
	"UY WUZ HERE", /* :] */
	"楽しい事をしていると時は早くたつ．", /* Who said this first, anyway? */
	"冗談じゃない！", /* Clueless */
	"How bizarre, how bizarre.", /* OMC */
	"Silly rabbit, Trix are for kids!", /* Trix */
 	"また戻る！", /* Terminator */
...
#else
	    	unsigned len, maxelen = BUFSZ - sizeof("あなたは次のように感じた：「」");
#endif
	    	len = strlen(ep->engr_txt);
	    	if (len > maxelen) {
#if 1 /*JP*/
	    		if (is_kanji2(ep->engr_txt, maxelen)) maxelen--;
#endif
...
*/
		pline("%sが空いていないので書けない！", body_part(HAND));
		return(0);
	}
...
#else
		    else Your("%sは起動していない！", xname(otmp));
#endif
		} else
#endif
		switch (otmp->otyp)  {
...
#else
		    pline("%sは輝いたが，徐々に消えていった．", xname(otmp));
#endif
		return(1);
	    } else {
		pline(Never_mind);
...
*/
			    You("傷をつけてしまったため，無力化してしまったため，賠償しなければならない！");
...
static const char *epitaphs[] = {
	...
	"郵便屋さん．電報は下記の住所に転送してください．ゲヘナ市アスモデウス要塞，左から5番目の死霊宛．",
	...
	"立ちされ！",
	/* From SLASH'EM */
	"このおじいさん　１つ　たたいた　ぼくの　おやゆびのうえで　とんとん　たたいた"
#endif
};
