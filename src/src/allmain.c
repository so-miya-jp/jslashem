...
*/
	        pline("ÉâÉbÉLÅ[ÅIç°î”ÇÕñûåéÇæÅD");
	change_luck(1);
    } else if(flags.moonphase == NEW_MOON) {
/*JP
	pline("Be careful!  New moon tonight.");
*/
	        pline("íçà”ÇµÇÎÅIç°î”ÇÕêVåéÇæÅD");
    }
...
*/
	        pline("ópêSÇµÇÎÅIÇPÇRì˙ÇÃã‡ójì˙Ç…ÇÕÇÊÇ≠Ç»Ç¢Ç±Ç∆Ç™Ç†ÇÈÅD") ;
	change_luck(-1);
    }
    /* KMH -- February 2 */
...
*/
	pline("ç°ì˙ÇÕçKÇπÇ»êπêCêﬂÇÃì˙ÅIètÇÃìûóàÅI");
...
*/
			    You("ónä‚Ç…ê[Ç≠íæÇ›ÅCónÇØÇΩÅD");
		    done(DISSOLVED);
		} else if(didmove && !u.umoved) {
/*JP
		    Norep("You sink deeper into the lava.");
*/
			    Norep("Ç†Ç»ÇΩÇÕónä‚ÇÃíÜÇ…íæÇ›çûÇÒÇ≈Ç¢Ç≠ÅD");
		    u.utrap += rnd(4);
		}
	    }
	}
...
*/
	Sprintf(eos(buf), "ÇÃ%s", genders[currentgend].j);
...
#else /*JP*/
    if(new_game){
	pline("%sÅC%sÇÃê¢äEÇ÷ÅIÇ±ÇÃÉQÅ[ÉÄÇ≈ÇÕÇ†Ç»ÇΩÇÕ%sÇÃ%s(%s)ÇæÅD",
	      Hello((struct monst *) 0, TRUE), DEF_GAME_NAME, urace.j,
	      (currentgend && urole.jname.f) ? urole.jname.f : urole.jname.m,
	      buf);
    } else {
	pline("%sÅC%sÇÃê¢äEÇ÷ÅIÇ†Ç»ÇΩÇÕ%sÇÃ%sÇæÅI",
	      Hello((struct monst *) 0, TRUE), DEF_GAME_NAME, urace.j,
	      (currentgend && urole.jname.f) ? urole.jname.f : urole.jname.m);
    }
#endif /*JP*/
