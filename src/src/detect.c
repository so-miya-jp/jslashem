...
*/
    You("どん欲になったような気がした，そして金の位置を感知した！");
    exercise(A_WIS, TRUE);
...
#else
	    Sprintf(buf, "あなたの%sがひくひくと動い%sた．", body_part(NOSE),
			(sobj->blessed && !u.uedibility) ? "て，うずうずしだし" : "");
#endif
...
*/
	case 2 : pline("覗いていた%sに混乱させられた！", xname(obj));
...
*/
		pline("覗いていた%sに視力を奪われた！", xname(obj));
...
*/
		pline("覗いていた%sに視界を狙われた．", xname(obj));
/*JP
		You("are unaffected!");
*/
...
*/
	case 4 : pline("覗いていた%sに精神を攻撃された！", xname(obj));
...
*/
	    case 5 : You("小さい雪片がミニチュアの農家のまわりを舞っているのを見た．");
		break;
/*JP
