...
static struct trans_verb trans_verb_list[] = {
    ...
    {"dip into", "どれ", "に", "浸す"},
    {"dip",	"どれ", "を", "浸す"},
    {"draw blood with", "どれ", "で", "採血する"},
    {"dress your wounds with", "どれ", "を", "傷に巻く"},
    {"drink",	"どれ", "を", "飲む"},
    {"drop",	"どれ", "を", "置く"},
    {"eat",	"どれ", "を", "食べる"},
    {"grease",	"どれ", "に", "塗る"},
    {"have identified",	"どれ", "を", "識別する"},
    {"improve",	"どれ", "を", "強化する"},
    {"invoke",	"ど", "の", "魔力を使う"},
    {"name",	"どれ", "を", "名づける"},
    {"pawn",	"どれ", "を", "質に入れる"},
...
#else
				You("%sいるため%sものに%sが届かない．", Levitation ? "浮いて" : "飛んで", jword, body_part(HAND));
#endif
			else
#if 0 /*JP*/
				pline("There's nothing here to %s.", word);
#else
				pline("そこには%sものがない．", jword);
#endif
			return(struct obj *)0;
		    }
#if 0 /*JP*/
...
*/
			...
			You("don't have that many!  You have only %ld.",
			    otmp->quan);
#else
			You("そんなに沢山は持っていない！せいぜい%ld%sってとこだ．",
			    otmp->quan, numeral(otmp));
#endif
#ifdef REDO
			if (in_doagain) return((struct obj *) 0);
#endif
...
/*JP CHECK: 0.0.7E7 の呼び出し元
do.c:946:		(result = ggetobj("drop", drop, 0, FALSE, (unsigned *)0)) < -1)
do.c:1007:	i = ggetobj("drop", drop, 0, TRUE, &ggoresults);
do_wear.c:2780:	    (result = ggetobj("take off", select_off, 0, FALSE, (unsigned *)0)) < -1)
do_wear.c:2828:	if (ggetobj("take off", select_off, 0, TRUE, (unsigned *)0) == -2)
invent.c:2076:		n = ggetobj("identify", identify, id_limit, FALSE, (unsigned *)0);
*/
int
...
invent.c:1846:ggetobj() int cnt = askchain(&invent, olets, allflag, fn, ckfn, mx, word); 
pickup.c:2808:("nodot") if (askchain((struct obj **)&current_container->cobj,
pickup.c:2906:("nodot") (void) askchain((struct obj **)&invent,
  wordには動詞が英語で入る．
*/
int
...
*/
    putstr(win, 0, xprname((struct obj *)0, "合計：", '*', FALSE, totcost, 0L));
...
*/
	    case D_BROKEN:	dfeature = "壊れた扉"; break;
...
#else
	if (cmap >= 0) 
	    dfeature = jtrns_obj('S', defsyms[cmap].explanation);
#endif
...
#else
	    putstr(tmpwin, 0, Blind ? "ここにあるらしいものは．．．" :
				      "ここにあるものは．．．");
#endif
...
#else
	    You("%sに武器をもっていない．", body_part(HAND));
#endif
	    return 0;
	}
...
#else
	    You("どちらの%sにも武器をもっていない．", body_part(HAND));
#endif
	    return 0;
	}
#if 0 /*JP*/
...
#else
	Your("右%sに武器をもっていない．", body_part(HAND));
#endif
    } else {
...
#else
    	    Your("もう片方の%sに武器を持っていない．", body_part(HAND));
#endif
    }
    return 0;
#if 0
