...
*/
		Sprintf(eos(linebuf), "%s", jtrns_obj('d', dungeons[t1->deathdnum].dname));
		if (t1->deathdnum != knox_level.dnum)
/*JP
		    Sprintf(eos(linebuf), " on level %d", t1->deathlev);
*/
		    Sprintf(eos(linebuf), "の地下%d階にて", t1->deathlev);
