...
STATIC_OVL boolean FDECL(cursed_object_at, (struct monst *, int, int));
...
boolean devour;
{
	...
	if (vis)
	    ...
	    pline("%sは%s%s．", mon_visible(mtmp) ? noit_Monnam(mtmp) : "何か",
		  (obj->oclass == FOOD_CLASS) ?
			singular(obj, doname) : doname(obj),
		  vampiric ? "に噛み付いている" : devour ? "をむさぼり食っている" : "を食べている");
#endif
...
#else
	    pline("%sはあなたに敵対した！", Monnam(mtmp));
#endif
	else
#if 0 /*JP*/
	    pline("You feel uneasy about %s.", y_monnam(mtmp));
#else
	    pline("あなたは%sを不安に感じた．", y_monnam(mtmp));
#endif
	mtmp->mpeaceful = 0;
	mtmp->mtame = 0;
	mtmp->mtraitor = TRUE;
