...
STATIC_VAR const char * const A_gush_of_water_hits = "水が噴出して";
STATIC_VAR const char * const blindgas[6] = 
...
*/
	static const char txt[] = "塗り込まれた脂によって守られた！";
...
*/
	    dont_fall = "大きすぎて穴に引っかかった．";
	} else if (!next_to_u()) {
/*JP
	    dont_fall = "are jerked back by your pet!";
*/
	    dont_fall = "ペットによって引っぱりもどされた！";
	}
	if (dont_fall) {
	    You(dont_fall);
...
*/
	    Sprintf(msgbuf, "%sに開いていた穴は閉じてしまった．",
		    ceiling(u.ux,u.uy));
...
#else
	    	pline("%sが%sた！", upstart(statuename),
	    		canspotmon(mon) ? comes_to_life : "消え失せ");
#endif
	    else
#if 0 /*JP*/
		pline_The("statue %s!",
...
*/
		    pline("%sあなたに命中した！", A_gush_of_water_hits);
/*JP
...
*/
			pline("%sあなたの%sに命中した！", A_gush_of_water_hits,
				    body_part(HEAD));
/*JP
...
*/
			pline("%sあなたの左%sに命中した！", A_gush_of_water_hits,
				    body_part(ARM));
/*JP
...
*/
			pline("%sあなたの右%sに命中した！", A_gush_of_water_hits,
				    body_part(ARM));
...
*/
		   pline("あなた%s%s落し穴に落ちた！", verbbuf, dig_you[trap->madeby_u]);
		}
		/* wumpus reference */
...
*/
		    /* pitiful(哀れ)とpit fill(穴埋め)を掛けているのだと思うが… */
		    /* 直訳風味 pline("哀れな．それは穴じゃないのか？"); */
...
*/
		...
		if (amorphous(youmonst.data) || is_whirly(youmonst.data) ||
			...
			if (webmsgok)
				...
				(u.umonnum == PM_FIRE_ELEMENTAL) ? "焼いた" : "溶かした");
#endif
			deltrap(trap);
			newsym(u.ux,u.uy);
			break;
		...
		if (webmsgok) {
		   ...
		   if (u.usteed)
		    ...
		    Sprintf(verbbuf, "%s", Levitation ? (const char *)"浮きながら" :
		      		locomotion(youmonst.data, "つまずいて"));
#endif
#if 0 /*JP*/
...
#else
		 Sprintf(verbbuf,"%s込んだ",
		    Levitation ? (const char *)"浮いたまま飛び" : 
...
#else
		    if(already_seen)
		      pline("ここには%s%sの起爆スイッチがある．",
			    set_you[trap->madeby_u], jtrns_obj('^', "land mine"));
		    else
		      You("足元の土の中に%s%sの起爆スイッチをみつけた．",
			    set_you[trap->madeby_u], jtrns_obj('^', "land mine"));
#endif
		    if (already_seen && rn2(3)) break;
#if 0 /*JP*/
...
#else
		    pline("ちゅどーん！！%s%s%sが反応した！",
			    forcebungle ? "不器用なせいで" :
			    		  "空気の流れに",
			    already_seen ? set_you[trap->madeby_u] : "",
			    already_seen ? jtrns_obj('^', "land mine") : "何かのスイッチ");
#endif
		} else {
#ifdef STEED
...
#else
				if (cansee(bhitpos.x, bhitpos.y))
				    pline("ちゅどーん！！転がってきた岩が%sによって爆破された．",
					  jtrns_obj('^', "land mine"));
				else
				    pline("ちゅどーん！！");
#endif
...
#else
			if(cansee(bhitpos.x, bhitpos.y))
			    pline("%s大きな音を立てた！", bmsg);
			else
			    You_hear("巨大な物体が激突する音を聞いた！");
#endif
...
*/
		pline("%sは気力が弱まったようだ！", Monnam(mtmp));
	mtmp->m_en -= n;
	if(mtmp->m_en < 0)  {
...
*/
			pline("%sはトゲトゲを食べている！", Monnam(mtmp));
		    trap->ttyp = PIT;
		    mtmp->meating = 5;
		}
	    }
...
#else
				pline("%s，%sの%sに命中した！", A_gush_of_water_hits,
				    mon_nam(mtmp), mbodypart(mtmp, HEAD));
#endif
...
*/
				pline("%s，%sの左%sに命中した！", A_gush_of_water_hits,
...
*/
				pline("%s，%sの右%sに命中した！", A_gush_of_water_hits,
				    mon_nam(mtmp), mbodypart(mtmp, ARM));
...
*/
				pline("%s，%sに命中した！", A_gush_of_water_hits,
				    mon_nam(mtmp));
...
*/
	pline("%sの足元の土の山に起爆スイッチが現れた．", mon_nam(mtmp));
					seetrap(trap);
				}
				if (rn2(3)) break;
				if (in_sight) {
				...
				Monnam(mtmp), a_your[trap->madeby_u]);
#else
			    pline("ちゅどーん！！%sは%s%sの起爆スイッチを踏んだ！",
				Monnam(mtmp), set_you[trap->madeby_u], jtrns_obj('^', "land mine"));
#endif
			}
			if (!in_sight)
/*JP
...
const char *arg;
{
	...
	if(uwep && uwep->otyp == CORPSE && touch_petrifies(&mons[uwep->corpsenm])
		...
		Sprintf(kbuf, "%sの死体に触れて", jtrns_mon(mons[uwep->corpsenm].mname));
#endif
		instapetrify(kbuf);
	}
...
*/
			Your("体は上に引っぱられたが，%sはまだはまっている．",
			     makeplural(body_part(LEG)));
		} else {
/*JP
			You("float up, only your %s is still stuck.",
*/
			You("浮き上がったが，片%sがまだはまっている．",
				body_part(LEG));
		}
	}
...
*/
	    	pline("%sは魔法の力で浮き上がった！", Monnam(u.usteed));
	    else {
/*JP
...
#else
	    You("下降したが，いまだ%s込まれたままだ．",
		is_animal(u.ustuck->data) ? "飲み" : "巻き");
#endif
	    return(1);
	}
...
*/
	    else losehp(rnd(3), "沸騰した水をかぶって", KILLED_BY);
	    return;
	}
#if 0 /*JP*/
...
*/
	    losehp(num, "火柱に巻き込まれて", KILLED_BY_AN);
	burn_away_slime();
...
*/
					losehp(dmg, "激しく揺れたナップザックで", KILLED_BY);
			break;
			}
#if 0 /*JP*/
...
*/
	    if (in_sight) pline("%sに火がついて燃えた．", Yname2(obj));
	    if (Has_contents(obj)) {
/*JP
		if (in_sight) pline("Its contents fall out.");
*/
		if (in_sight) pline("中身が転げ落ちた．");
...
drown()
{
...
#else
	const char *sparkle = level.flags.lethe? "泡立つ" : "";
#endif
	...
	if (level.flags.lethe) {
	    ...
	    You_feel("the sparkling waters of the Lethe sweep away your "
			    "cares!");
#else
	    You("レーテ河の泡立つ水が記憶を洗い流してゆくような気がする！");
#endif
	    forget(25);
	}
...
*/
	    pline("で，どうやってそれをするつもりなんだい？");
	    return 0;
	} else if (u.ustuck && sticks(youmonst.data)) {
/*JP
	    pline("You'll have to let go of %s first.", mon_nam(u.ustuck));
*/
	    pline("あなたはまず，%sを放してやらねばならないだろう．", mon_nam(u.ustuck));
	    return 0;
	}
...
#else
	    Your("%sではそんなことをする余裕はなさそうだ．", body_part(HAND));
#endif
	    return 0;
	}
	return untrap(FALSE);
...
#else
		You("%s%sを%sからはずした",
			(ttmp->ttyp == BEAR_TRAP) ? set_you[ttmp->madeby_u] : web_you[ttmp->madeby_u],
...
*/
			You("%sくもの巣を取り除いた．", web_you[ttmp->madeby_u]);
			deltrap(ttmp);
		}
	}
...
*/
	You("%s%sを解除した．", set_you[ttmp->madeby_u],
		jtrns_obj('^', "land mine"));
...
*/
	You("%sを解除した！", jtrns_obj('^', "rust trap"));
	deltrap(ttmp);
...
*/
	...
	if (uprob) {
		...
		mon_nam(mtmp));
#else
	    You("%sを引き上げようとしたが，しっかりと掴むことができなかった．",
		mon_nam(mtmp));
#endif
	    if (mtmp->msleeping) {
		mtmp->msleeping = 0;
...
*/
	You("%sを落し穴から引っぱり出した．", mon_nam(mtmp));
	mtmp->mtrapped = 0;
	fill_pit(mtmp->mx, mtmp->my);
	reward_untrap(ttmp, mtmp);
	return 1;
...
*/
...
#else
			    Sprintf(qbuf, "箱と%sがある．%s？", the_trap,
		...
		if (u.utrap) {
			...
			You("cannot deal with %s while trapped%s!", the_trap,
				(x == u.ux && y == u.uy) ? " in it" : "");
#else
			pline("罠にかかったままでは%sを解除できない！", the_trap);
#endif
			return 1;
		}
		switch(ttmp->ttyp) {
...
#else
		    Sprintf(qbuf, "%sがある．罠を調べますか？",
			safe_qbuf("", sizeof("がある．罠を調べますか？"),
...
*/
	You(disarm ? "罠を作動させてしまった！" : "罠にひっかかった！");
	display_nhwindow(WIN_MESSAGE, FALSE);
...
*/
	You("いつのまにか固い%sの上に戻っていた．", surface(u.ux, u.uy));
	return(TRUE);
    }
