...
#else
		pline("それは%s輝いた！", jconj_adj(hcolor(clr)));
#endif
	    return 1;
	}
...
*/
		You_cant("透明な魔法書の神秘的なルーン文字を読むことができない．");
		return(0);
	    }
	    else if (!scroll->dknown)
	    {
...
#else
		pline("混乱していたので，あなたは魔法の言葉を間違って%sてしまった．．．",
			is_silent(youmonst.data) ? "念じ" : "唱え");
#endif
	  }
	}
...
*/
		    pline("『Larnの瞳』の発見者に宛てられたジャンクメールのようだ．");
		/* note to the puzzled: the game Larn actually sends you junk
		 * mail if you win!
		 * JP : RubyLarnにおける"the Eye of Larn"の翻訳に従いました．
		 */
		else readmail(sobj);
		break;
#endif
...
#else
			    Your("%sは%s%sで覆われた！", xname(otmp),
				hcolor(sobj->cursed ? NH_BLACK : NH_GOLDEN),
				sobj->cursed ? "まだらの輝き" : "ゆらめくバリア");
#endif
			}
			if (otmp->oerodeproof &&
...
*/
		You("巻物に魔法の言葉が書かれてなかったことを思いだした．");
	    else
/*JP
		pline("This scroll seems to be blank.");
*/
		pline("この巻物には何も書かれてないようだ．");
	    known = TRUE;
	    break;
...
*/
				if (mtmp) You("%sを支配した！", mon_nam(mtmp));
			    }
			} else setmangry(mtmp);
		}
		multi = oldmulti;
...
*/
			    You("不死の怪物に命令できる呪文を覚えていないようだ！");
		    } else You("不死の怪物に命令する術を知らない．．．");
		}
...
*/
		    You("何かが呪文をさえぎり，%sがまわった！", body_part(EYE));
		    make_confused(HConfusion + rnd(30), FALSE);
		    break;
		}
/*JP
...
*/
			    pline("ほら見て．あなたの%sの中にとてもかわいらしい炎があるよ．",
				makeplural(body_part(HAND)));
/*JP
...
*/
		    getlin("どのクラスに属する怪物を虐殺しますか？[?でヘルプ]",
			buf);
		    (void)mungspaces(buf);
...
*/
	pline("そのクラスの怪物はもう存在しない．");
			else if (immunecnt ||
				(buf[0] == DEF_INVISIBLE && buf[1] == '\0'))
/*JP
	You("aren't permitted to genocide such monsters.");
*/
	You("そのクラスの怪物を虐殺することはできない．");
			else
...
*/
	    getlin("どの種の怪物を作りますか？[名前かシンボルを入れてね]",
		   buf);
	    bufp = mungspaces(buf);
#if 1 /*JP*/
	    {
