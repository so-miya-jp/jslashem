/*
**
**	$Id: jconj.c,v 1.7 2006/11/11 10:29:17 so-miya Exp $
**
*/
...
/*	  memcpy(p+2, e2sj(sj2e(p+2)+1), 2);*//* sj2e() returns ptr to char* */
	...
	else if(!strncmp(sfx, "‚Ü", 2) || !strncmp(sfx, "‚Í", 2)) {
	    if(!IC){
		p[1] = hira_tab[tab->column + 1][1];
	    } else {
		memcpy(p, e2sj(hira_tab[tab->column + 1]), 2);
	...
	else if(!strncmp(sfx, "‚ê", 2) && strncmp(sfx, "‚ê‚Î", 4) &&
		tab->katsuyo_type == LOWER){
	    strcpy((char *)p, "‚ç");
	    strcpy((char *)p + 2, sfx);
	}
	else
...
main()
{
	...
	printf("%-10s(˜A—p)‚Ü‚·    %s\n", tab->main, jpolite(tab->main));
	printf("%-10s(˜A—p)‚Í‚¶‚ß  %s\n", tab->main, jconj(tab->main, "‚Í‚¶‚ß‚é"));
	printf("%-10s(˜A‘Ì)‚Æ‚«    %s\n", tab->main, jconj(tab->main, "‚Æ‚«"));
	printf("%-10s(‰¼’è)‚ê‚Î    %s\n", tab->main, jconj(tab->main, "‚ê‚Î"));
	printf("%-10s(ˆÓŒü)‚æ‚¤    %s\n", tab->main, jconj(tab->main, "‚æ‚¤"));
	printf("%-10s(‰ß‹Ž)‚½      %s\n", tab->main, jpast(tab->main));
