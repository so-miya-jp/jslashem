/*
  $Id: gtkmenu.c,v 1.6 2005/05/12 14:43:03 so-miya Exp $
 */
/*
...
#else
	b = w->button[2] = nh_gtk_new_and_pack(
	  GTK_j_button_new_with_label("全選択"), w->hbox, "",
	  FALSE, FALSE, NH_PAD);
#endif
	gtk_signal_connect(GTK_OBJECT(b), "clicked",
...
#else
	b = w->button[3] = nh_gtk_new_and_pack(
	  GTK_j_button_new_with_label("選択解除"), w->hbox, "",
	  FALSE, FALSE, NH_PAD);
#endif
	gtk_signal_connect(GTK_OBJECT(b), "clicked",
...
#else
	b = w->button[4] = nh_gtk_new_and_pack(
	  GTK_j_button_new_with_label("選択反転"), w->hbox, "",
	  FALSE, FALSE, NH_PAD);
#endif
	gtk_signal_connect(GTK_OBJECT(b), "clicked",
...
#else
	GTK_j_frame_set_label(GTK_FRAME(w->frame),
	  w->menu_information->curr_menu.prompt);
#endif
...
#else
	    text[0] = g_locale_to_utf8("お金以外何も持っていない．", -1, NULL, NULL, NULL );
#endif
	else
#if 0 /*JP*/
	    text[0] = "Not carrying anything.";
#else
	    text[0] = g_locale_to_utf8("何も持っていない．", -1, NULL, NULL, NULL );
#endif
...
#else
    utf8_message = g_locale_to_utf8( str, -1, NULL, NULL, NULL );
    text[MENU_COLS-1] = utf8_message ;
#endif
