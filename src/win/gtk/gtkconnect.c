...
#else
  label1 = GTK_j_label_new ("接続するサーバを選択するか，新しいサーバを設定してください．");
#endif
...
#else
  button1 = GTK_j_button_new_with_mnemonic ("追加(_A)...");
#endif
...
#else
  button2 = GTK_j_button_new_with_mnemonic ("編集(_E)...");
#endif
...
#else
  button3 = GTK_j_button_new_with_mnemonic ("削除(_D)");
#endif
...
#else
  button4 = GTK_j_button_new_with_mnemonic ("デフォルトに設定(_S)");
#endif
...
#else
  label13 = GTK_j_label_new_with_mnemonic ("接続(_o)");
#endif
...
#else
  GTK_j_window_set_title (GTK_WINDOW (AddConnection), "接続の追加");
#endif
...
#else
  label9 = GTK_j_label_new ("接続サーバ名を選択してください．");
#endif
...
#else
  label10 = GTK_j_label_new_with_mnemonic ("接続名(_C):");
#endif
...
#else
  label3 = GTK_j_label_new ("接続したいサーバタイプを選択してください：");
#endif
...
#else
  label4 = GTK_j_label_new_with_mnemonic ("サーバタイプ(_S)：");
#endif
...
#else
  label14 = GTK_j_label_new ("トラブル解決");
#endif
...
#else
  DisableAsync = GTK_j_check_button_new_with_mnemonic ("非同期サポートを無効化する(_a) (待ち時間が長くなるでしょう)");
#endif
...
#else
  EnableLogging = GTK_j_check_button_new_with_mnemonic ("ログ出力を有効にする(_l)");
#endif
...
#else
  GTK_j_window_set_title (GTK_WINDOW (RemoteMachine), "リモート接続設定");
#endif
...
#else
  label6 = GTK_j_label_new_with_mnemonic ("ポート(_P)：");
#endif
...
#else
  label11 = GTK_j_label_new_with_mnemonic ("ホスト名(_H)：");
#endif
...
#else
  GTK_j_window_set_title (GTK_WINDOW (LocalExecutable), "ローカル実行環境の設定");
#endif
...
#else
  label12 = GTK_j_label_new ("実行するサーバ名を入力してください．");
#endif
...
#else
  button6 = GTK_j_button_new_with_mnemonic ("選択(_B)...");
#endif
...
#else
  GTK_j_window_set_title (GTK_WINDOW (ConnectTerminal), "GtkHack: 接続失敗");
#endif
...
#else
  GTK_j_window_set_title (GTK_WINDOW (Troubleshooting), "トラブル解決");
#endif
...
#else
  label15 = GTK_j_label_new ("ゲームサーバへの接続に関するトラブルが発生していますか？");
#endif
...
#else
  label16 = GTK_j_label_new ("これらの設定はgtkhackがどうサーバに接続するかを制御します．");
#endif
...
#else
  label17 = GTK_j_label_new ("接続に関する問題の解決を手助けします．");
#endif
...
#else
  DisableAsync = GTK_j_check_button_new_with_mnemonic ("非同期サポートの無効化(_a)をしてみる");
#endif
  gtk_widget_set_name (DisableAsync, "DisableAsync");
  gtk_widget_show (DisableAsync);
...
#else
  ViewLog = GTK_j_button_new_with_mnemonic ("ログを参照する(_V)");
#endif
...
#else
  GTK_j_window_set_title (GTK_WINDOW (ViewLog), "接続ログの参照");
#endif
...
#else
  menuitem4 = GTK_j_menu_item_new_with_mnemonic ("ファイル(_F)");
#endif
...
#else
  menuitem5 = GTK_j_menu_item_new_with_mnemonic ("編集(_E)");
#endif
