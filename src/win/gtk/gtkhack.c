/* $Id: gtkhack.c,v 1.5 2005/05/12 14:43:40 so-miya Exp $ */
...
static void GTK_proxy_clnt_errhandler(const char *error)
{
    ...
    w = gtk_message_dialog_new(NULL, 0, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
      error);
#else
    gchar *utf8_error = g_locale_to_utf8(error, -1, NULL, NULL, NULL );
    w = gtk_message_dialog_new(NULL, 0, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
      utf8_error);
    g_free(utf8_error);
#endif
...
#else
	gchar *utf8_message = g_locale_to_utf8(
	  "ゲームサーバへの接続が切断されました．\n\n%s", -1, NULL, NULL, NULL );
	gchar *utf8_error = g_locale_to_utf8(error, -1, NULL, NULL, NULL );

	w = gtk_message_dialog_new(NULL, 0, GTK_MESSAGE_ERROR, GTK_BUTTONS_NONE,
	  utf8_message, utf8_error);
	g_free(utf8_error);
	g_free(utf8_message);
#endif
...
#else
	s = g_strdup_printf("ゲームサーバが認証を要求していますが，\n"
	  "このタイプの認証には対応していません．\n\n"
	  "対応している認証方法： 0,1\n"
	  "要求された認証方法： %s",win_proxy_clnt_gettag("authmethods"));
#endif
...
main(int argc, char **argv)
{
    ...
    gulong flags;
#if 1 /*JP*/
    gchar *utf8_name, *utf8_scheme, *utf8_address;
#endif
...
#else
	GTK_connection_add("local", "file", "jslashem", 0UL);
#endif
...
#else
    utf8_name = g_locale_to_utf8( "接続名", -1, NULL, NULL, NULL );
    column = gtk_tree_view_column_new_with_attributes(utf8_name, renderer,
      "text", COLUMN_NAME, NULL);
    g_free(utf8_name);
#endif
...
#else
    utf8_scheme = g_locale_to_utf8( "形態", -1, NULL, NULL, NULL );
    column = gtk_tree_view_column_new_with_attributes(utf8_scheme, renderer,
      "text", COLUMN_SCHEME, NULL);
    g_free(utf8_scheme);
#endif
    gtk_tree_view_column_set_resizable(column, TRUE);
...
#else
    utf8_address = g_locale_to_utf8( "アドレス", -1, NULL, NULL, NULL );
    column = gtk_tree_view_column_new_with_attributes(utf8_address, renderer,
      "text", COLUMN_ADDRESS, NULL);
    g_free(utf8_address);
#endif
    gtk_tree_view_column_set_resizable(column, TRUE);
	...
	if (gtk_tree_selection_get_selected(sel, &model, &iter)) {
		...
		gchar *utf8_message = g_locale_to_utf8(
		  "%sへの接続を開始します．\n\n接続を確立中…",
		  -1, NULL, NULL, NULL );
		progress = gtk_message_dialog_new(NULL, 0, GTK_MESSAGE_INFO, 
		  GTK_BUTTONS_CANCEL, utf8_message, name);
		g_free(utf8_message);
	    }
#endif
#if 0 /*JP*/
	    gtk_window_set_title(GTK_WINDOW(progress),
	      "GtkHack: Connecting...");
#else
	    GTK_j_window_set_title(GTK_WINDOW(progress),
	      "GtkHack: 接続中…");
#endif
		...
		if (os_path) {
		    ...
		    {
			gchar *utf8_message = g_locale_to_utf8(
			  "OSに特化したファイルパスに変換できません．\n\n%s",
			  -1, NULL, NULL, NULL );
			w = gtk_message_dialog_new(GTK_WINDOW(progress),
			  GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR,
			  GTK_BUTTONS_CLOSE, utf8_message, err->message);
			g_free(utf8_message);
		    }
#endif
		    gtk_dialog_run(GTK_DIALOG(w));
