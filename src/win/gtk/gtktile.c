/*
  $Id: gtktile.c,v 1.5 2005/12/09 17:44:14 so-miya Exp $
 */
/*
...
#else
    nh_gtk_progress_window_add_stage(w, "glyphを転送中");
    nh_gtk_progress_window_add_stage(w, "タイルセットを転送中");
    nh_gtk_progress_window_add_stage(w, "タイルをglyphに割り当て中");
#endif
}
...
#else
	nh_gtk_progress_window_add_stage(w, "タイルセットマッピングを転送中");
#endif
    g_free(name);
    free(md5sum);
}
...
#else
    nh_gtk_progress_window_add_stage(w, "タイルセット画像を転送中");
#endif
#else
#if 0 /*JP*/
    nh_gtk_progress_window_add_stage(w, "Loading tileset images");
#else
    nh_gtk_progress_window_add_stage(w, "タイルセット画像を読み込み中");
#endif
#endif
#if 0 /*JP*/
    nh_gtk_progress_window_add_stage(w, "Converting images");
#else
    nh_gtk_progress_window_add_stage(w, "画像を変換中");
#endif
}
