/*
  $Id: gtk.c,v 1.16 2009/07/21 15:37:44 so-miya Exp $
 */
/*
...
#include <signal.h>
#ifdef WIN32
#define _OBJC_NO_COM	/* MinGW 3.4 redefines "boolean" in windows.h */
#include <windows.h>
#undef _OBJC_NO_COM
#endif
#include "md5.h"
...
#if 1 /*JP*/
static void create_utf8_menu(void);
static void destroy_utf8_menu(void);
#endif
...
#else
    return GTK_getline("あなたは誰？ ", FALSE);
#endif
}
...
#else /*JP*/
#define MENU_GAME "/ゲーム"
#define MENU_PLAY "/開始"
#define MENU_SAVE "/保存"

static GtkItemFactoryEntry *mainmenu_items = NULL;
static int nmainmenu_items = 0;
static GtkItemFactoryEntry locale_mainmenu_items[] = {
    {MENU_GAME,				NULL,		NULL,		0,	"<Branch>"},
    {MENU_GAME "/Gtear1",		NULL,		NULL,		0,	"<Tearoff>"},
    {MENU_GAME MENU_PLAY,		NULL,		NULL,		0,	"<Branch>"},
    {MENU_GAME "/Gsep1",		NULL,		NULL,		0,	"<Separator>"},
    {MENU_GAME MENU_SAVE,		"<shift>S",	key_command,	'S',	NULL},
    {MENU_GAME "/ゲーム設定",		"<shift>O",	game_option,	'O',    NULL},
    {MENU_GAME "/カスタマイズ",		NULL,		game_preferences, 0,    NULL},
    {MENU_GAME "/スコア",		NULL,		game_topten,	0,	NULL},
#ifdef GTKHACK
    {MENU_GAME "/トラブルの解決",	NULL,		game_troubleshooting, 0,NULL},
#endif
    {MENU_GAME "/Gsep2",		NULL,		NULL,		0,	"<Separator>"},
    {MENU_GAME "/終了",			NULL,		game_quit,	0,	NULL},
};
#endif /*JP*/
...
static GtkItemFactoryEntry *helpmenu_items = NULL;
static int nhelpmenu_items = 0;
static GtkItemFactoryEntry locale_helpmenu_items[] = {
    {MENU_HELP,				NULL,		NULL,		0,	"<LastBranch>"},
    {MENU_HELP "/Htear1",		NULL,		NULL,		0,	"<Tearoff>"},
    {MENU_HELP "/コマンドヘルプ",	NULL,		help_help,	0,	NULL},
    {MENU_HELP "/キーの説明",		NULL,		help_shelp,	0,	NULL},
    {MENU_HELP "/オプション説明",	NULL,		help_option,	0,	NULL},
    {MENU_HELP "/sep3",			NULL,		NULL,		0,	"<Separator>"},
    {MENU_HELP "/SLASH'EMの歴史",	NULL,		help_history,	0,	NULL},
    {MENU_HELP "/ライセンス",		NULL,		help_license,	0,	NULL},
};
#endif /*JP*/
...
#define MENU_FIGH "/戦闘"
#define MENU_CHEC "/確認"
#define MENU_EQUI "/装備品"
#define MENU_YOU  "/状態"
#define MENU_ADVE "/名付け"
#define MENU_ACTI "/行動"
#define MENU_RELI "/信仰"
#define MENU_SPEC "/特殊"

static GtkItemFactoryEntry *playmenu_items = NULL;
static int nplaymenu_items = 0;
static GtkItemFactoryEntry locale_playmenu_items[] = {
    {MENU_MOVE,					NULL,		NULL,		0,		"<Branch>"},
    {MENU_MOVE "/Mtear1",			NULL,		NULL,		0,		"<Tearoff>"},
    {MENU_MOVE "/北へ移動",			NULL,		move_command,	GTK_NORTH,	NULL},
    {MENU_MOVE "/東へ移動",			NULL,		move_command,	GTK_EAST,	NULL},
    {MENU_MOVE "/南へ移動",			NULL,		move_command,	GTK_SOUTH,	NULL},
    {MENU_MOVE "/西へ移動",			NULL,		move_command,	GTK_WEST,	NULL},
    {MENU_MOVE "/北東へ移動",			NULL,		move_command,	GTK_NORTHEAST,	NULL},
    {MENU_MOVE "/北西へ移動",			NULL,		move_command,	GTK_NORTHWEST,	NULL},
    {MENU_MOVE "/南東へ移動",			NULL,		move_command,	GTK_SOUTHEAST,	NULL},
    {MENU_MOVE "/南西へ移動",			NULL,		move_command,	GTK_SOUTHWEST,	NULL},
    {MENU_MOVE "/降りる",			"greater",	key_command,	'>',		NULL},
    {MENU_MOVE "/昇る",				"less",		key_command,	'<',		NULL},
    {MENU_FIGH,					NULL,		NULL,		0,		"<Branch>"},
    {MENU_FIGH "/Ftear1",			NULL,		NULL,		0,		"<Tearoff>"},
    {MENU_FIGH "/北にいる怪物と戦闘する",	NULL,		fight_command,	GTK_NORTH,	NULL},
    {MENU_FIGH "/東にいる怪物と戦闘する",	NULL,		fight_command,	GTK_EAST,	NULL},
    {MENU_FIGH "/南にいる怪物と戦闘する",	NULL,		fight_command,	GTK_SOUTH,	NULL},
    {MENU_FIGH "/西にいる怪物と戦闘する",	NULL,		fight_command,	GTK_WEST,	NULL},
    {MENU_FIGH "/北東にいる怪物と戦闘する",	NULL,		fight_command,	GTK_NORTHEAST,	NULL},
    {MENU_FIGH "/北西にいる怪物と戦闘する",	NULL,		fight_command,	GTK_NORTHWEST,	NULL},
    {MENU_FIGH "/南東にいる怪物と戦闘する",	NULL,		fight_command,	GTK_SOUTHEAST,	NULL},
    {MENU_FIGH "/南西にいる怪物と戦闘する",	NULL,		fight_command,	GTK_SOUTHWEST,	NULL},
    {MENU_CHEC,					NULL,		NULL,		0,		"<Branch>"},
    {MENU_CHEC "/Ctear1",			NULL,		NULL,		0,		"<Tearoff>"},
    {MENU_CHEC "/足元を確認する",		"colon",	key_command,	':',		NULL},
    {MENU_CHEC "/指定した位置に何があるか見る",	"semicolon",	key_command,	';',		NULL},
    {MENU_CHEC "/罠を確認する",			"asciicircum",	key_command,	'^',		NULL},
    {MENU_EQUI,					NULL,		NULL,		0,		"<Branch>"},
    {MENU_EQUI "/Etear1",			NULL,		NULL,		0,		"<Tearoff>"},
    {MENU_EQUI "/武器を装備する",		"w",		key_command,	'w',		NULL},
    {MENU_EQUI "/主武器を副武器と交換する",	"x",		key_command,	'x',		NULL},
    {MENU_EQUI "/二刀流をする",			NULL,		ext_command,	EXT_CMD_TWOWEAPON,	NULL},
    {MENU_EQUI "/矢弾を装填する",		"<shift>q",	key_command,	'Q',		NULL},
    {MENU_EQUI "/防具を装備する",		"<shift>w",	key_command,	'W',		NULL},
    {MENU_EQUI "/防具を脱ぐ",			"<shift>t",	key_command,	'T',		NULL},
    {MENU_EQUI "/装飾品を身につける",		"<shift>p",	key_command,	'P',		NULL},
    {MENU_EQUI "/装飾品を外す",			"<shift>r",	key_command,	'R',		NULL},
    {MENU_YOU,					NULL,		NULL,		0,		"<Branch>"},
    {MENU_YOU "/Ytear1",			NULL,		NULL,		0,		"<Tearoff>"},
    {MENU_YOU "/所持品一覧",			"i",		key_command,	'i',		NULL},
    {MENU_YOU "/装備している武器を確認",	"parenright",	key_command,	')',		NULL},
    {MENU_YOU "/装備している防具を確認",	"bracketleft",	key_command,	'[',		NULL},
    {MENU_YOU "/はめている指輪を確認",		"equal",	key_command,	'=',		NULL},
    {MENU_YOU "/身に付けている魔除けを確認",	"quotedbl",	key_command,	'"',		NULL},
    {MENU_YOU "/身に付けている道具を確認",	"parenleft",	key_command,	'(',		NULL},
    {MENU_YOU "/詠唱可能な呪文を確認",		"plus",		key_command,	'+',		NULL},
    {MENU_YOU "/識別したアイテムの種類",	"backslash",	key_command,	'\\',		NULL},
    {MENU_YOU "/挑戦項目一覧",			NULL,		ext_command,	EXT_CMD_CONDUCT,	NULL},
    {MENU_ADVE,					NULL,		NULL,		0,		"<Branch>"},
    {MENU_ADVE "/ADtear1",			NULL,		NULL,		0,		"<Tearoff>"},
    {MENU_ADVE "/アイテムに呼び名をつける",	"<alt>n",	key_command,	'n' | 0x80,	NULL},
    {MENU_ADVE "/ペット等に呼び名をつける",	"<shift>C",	key_command,	'C',		NULL},
    {MENU_ADVE "/アイテムに割り振られた文字を入れ替える", "<alt>a", key_command, 'a' | 0x80,	NULL},
    {MENU_ACTI,					NULL,		NULL,		0,		"<Branch>"},
    {MENU_ACTI "/ACtear1",			NULL,		NULL,		0,		"<Tearoff>"},
    {MENU_ACTI "/休憩する",			"period",	key_command,	'.',		NULL},
    {MENU_ACTI "/隠されたものを探索する",	"s",		key_command,	's',		NULL},
    {MENU_ACTI "/食べる",			"e",		key_command,	'e',		NULL},
    {MENU_ACTI "/Asep1",			NULL,		NULL,		0,		"<Separator>"},
    {MENU_ACTI "/扉を開ける",			"o",		key_command,	'o',		NULL},
    {MENU_ACTI "/扉を閉じる",			"c",		key_command,	'c',		NULL},
    {MENU_ACTI "/Asep2",			NULL,		NULL,		0,		"<Separator>"},
    {MENU_ACTI "/拾う",				"comma",	key_command,	',',		NULL},
    {MENU_ACTI "/落とす",			"d",		key_command,	'd',		NULL},
    {MENU_ACTI "/箱を開ける",			"<alt>l",	key_command,	'l' | 0x80,	NULL},
    {MENU_ACTI "/道具を使用する",		"a",		key_command,	'a',		NULL},
    {MENU_ACTI "/蹴り付ける",			"<control>D",	key_command,	'\04',		NULL},
    {MENU_ACTI "/アイテムを投げる",		"t",		key_command,	't',		NULL},
    {MENU_ACTI "/装填したものを撃つ",		"f",		key_command,	'f',		NULL},
    {MENU_ACTI "/Asep3",			NULL,		NULL,		0,		"<Separator>"},
    {MENU_ACTI "/飲む",				"q",		key_command,	'q',		NULL},
    {MENU_ACTI "/読む",				"r",		key_command,	'r',		NULL},
    {MENU_ACTI "/呪文を詠唱する",		"<shift>Z",	key_command,	'Z',		NULL},
    {MENU_ACTI "/杖を振る",			"z",		key_command,	'z',		NULL},
    {MENU_ACTI "/ひたす",			"<alt>d",	key_command,	'd' | 0x80,	NULL},
    {MENU_ACTI "/座る",				"<alt>s",	key_command,	's' | 0x80,	NULL},
    {MENU_RELI,					NULL,		NULL,		0, 		"<Branch>"},
    {MENU_RELI "/Rtear1",			NULL,		NULL,		0,		"<Tearoff>"},
    {MENU_RELI "/神に祈って助けを求める",	"<alt>p",	key_command,	'p' | 0x80,	NULL},
    {MENU_RELI "/神に供物を捧げる",		"<alt>o",	key_command,	'o' | 0x80,	NULL},
    {MENU_SPEC,					NULL,		NULL,		0,		"<Branch>"},
    {MENU_SPEC "/Stear1",			NULL,		NULL,		0,		"<Tearoff>"},
    {MENU_SPEC "/刻む",				"<shift>E",	key_command,	'E', 		NULL},
    {MENU_SPEC "/対価を支払う",			"p",		key_command,	'p', 		NULL},
    {MENU_SPEC "/お金を掏る",			"<alt>b",	key_command,	'b' | 0x80,	NULL},
    {MENU_SPEC "/話し掛ける",			"<alt>c",	key_command,	'c' | 0x80,	NULL},
    {MENU_SPEC "/熟練度を上げる/確認する",	"<alt>e",	key_command,	'e' | 0x80,	NULL},
    {MENU_SPEC "/こじ開ける",			"<alt>f",	key_command,	'f' | 0x80,	NULL},
    {MENU_SPEC "/アイテムの魔力を発動させる",	"<alt>i",	key_command,	'i' | 0x80,	NULL},
    {MENU_SPEC "/怪物の特殊能力を使う",		"<alt>m",	key_command,	'm' | 0x80,	NULL},
    {MENU_SPEC "/騎乗する",			NULL,		ext_command,	EXT_CMD_RIDE,	NULL},
    {MENU_SPEC "/擦る",				"<alt>r",	key_command,	'r' | 0x80,	NULL},
    {MENU_SPEC "/顔を拭う",			"<alt>w",	key_command,	'w' | 0x80,	NULL},
    {MENU_SPEC "/特殊技能を使用する",		"<alt>t",	key_command,	't' | 0x80,	NULL},
    {MENU_SPEC "/瞬間移動する",			"<control>t",	key_command,	'',		NULL},
    {MENU_SPEC "/アンデッドを祓う",		NULL,		ext_command,	EXT_CMD_TURN,	NULL},
    {MENU_SPEC "/罠を解除する",			"<alt>u",	key_command,	'u' | 0x80,	NULL},
    {MENU_SPEC "/別の位置にジャンプする",	"<alt>j",	key_command,	'j' | 0x80,	NULL},
};
#endif /*JP*/
...
#else
    nh_menu_sensitive(MENU_GAME, enable);
    nh_menu_sensitive(MENU_MOVE, enable);
    nh_menu_sensitive(MENU_FIGH, enable);
    nh_menu_sensitive(MENU_CHEC, enable);
    nh_menu_sensitive(MENU_EQUI, enable);
    nh_menu_sensitive(MENU_YOU, enable);
    nh_menu_sensitive(MENU_ADVE, enable);
    nh_menu_sensitive(MENU_ACTI, enable);
    nh_menu_sensitive(MENU_RELI, enable);
    nh_menu_sensitive(MENU_SPEC, enable);
    nh_menu_sensitive(MENU_HELP, enable);
#endif
    return 0;
}
...
#else
    gchar *utf8_message = g_locale_to_utf8( menu, -1, NULL, NULL, NULL );
    p = gtk_item_factory_get_widget(main_item_factory, utf8_message);
    gtk_widget_set_sensitive(p, f);
    g_free( utf8_message );
#endif
}
...
#else
#define PLAYER_ROLE_M(i)	(roles[i].jname.m)
#define PLAYER_ROLE_F(i)	(roles[i].jname.f)
#define PLAYER_RACE(i)		(races[i].j)
#define PLAYER_GENDER(i)	(genders[i].j)
#define PLAYER_ALIGN(i)		(aligns[i].j)
#endif /*JP*/
#endif	/* GTK_PROXY */
...
select_node_path(unsigned long key, int level, char *leaf)
{
    ...
    char *utf8_role = NULL;
    char *utf8_race = NULL;
    char *utf8_gender = NULL;
    char *utf8_align = NULL;
    char *utf8_root = g_locale_to_utf8( MENU_GAME MENU_PLAY, -1, NULL, NULL, NULL );
    char *utf8_random = g_locale_to_utf8( "ランダム", -1, NULL, NULL, NULL );
#endif
...
#else
	{
	    utf8_role = g_locale_to_utf8( PLAYER_ROLE_M(rolenum), -1, NULL, NULL, NULL );
	    len += strlen(utf8_role);
	}
#endif
	else
#if 0 /*JP*/
...
#else
	{
	    utf8_race = g_locale_to_utf8( PLAYER_RACE(racenum), -1, NULL, NULL, NULL );
	    len += strlen(utf8_race);
	}
#endif
	else
#if 0 /*JP*/
...
#else
	{
	    utf8_gender = g_locale_to_utf8( PLAYER_GENDER(gendnum), -1, NULL, NULL, NULL );
	    len += strlen(utf8_gender);
	}
#endif
	else
#if 0 /*JP*/
...
#else
	{
	    utf8_align = g_locale_to_utf8( PLAYER_ALIGN(alignnum), -1, NULL, NULL, NULL );
	    len += strlen(utf8_align);
	}
#endif
	else
#if 0 /*JP*/
...
#else
    strcpy(path, utf8_root);
#define SELECT_STR(num, str) (((num) >= 0) ? (str) : utf8_random)
#endif
    if (level > 0) {
	strcat(path, "/");
#if 0 /*JP*/
...
#else
	strcat(path, SELECT_STR(rolenum, utf8_role));
#endif
    }
    if (level > 1) {
	strcat(path, "/");
...
#else
	strcat(path, SELECT_STR(racenum, utf8_race));
#endif
    }
    if (level > 2) {
	strcat(path, "/");
...
#else
	strcat(path, SELECT_STR(gendnum, utf8_gender));
#endif
    }
    if (level > 3) {
	strcat(path, "/");
...
#else
	strcat(path, SELECT_STR(alignnum, utf8_align));
#endif
    }
    if (leaf) {
	strcat(path, "/");
    ...
    g_free(utf8_random) ;
    g_free(utf8_root) ;
    if ( utf8_align != NULL ) g_free(utf8_align) ;
    if ( utf8_gender != NULL ) g_free(utf8_gender) ;
    if ( utf8_race != NULL ) g_free(utf8_race) ;
    if ( utf8_role != NULL ) g_free(utf8_role) ;
#endif
#undef SELECT_STR
    return path;
}
...
#else
	gchar *utf8_message = g_locale_to_utf8( titles[level], -1, NULL, NULL, NULL );
	menu_items[offset].path =
	  select_node_path(node->key, level, utf8_message);
	g_free(utf8_message) ;
#endif
...
# endif
    ...
    gtk_item_factory_create_items(main_item_factory, SIZE(mainmenu_items),
      mainmenu_items, NULL);
#else
    gtk_item_factory_create_items(main_item_factory, nmainmenu_items,
      mainmenu_items, NULL);
#endif
    ...
    gtk_item_factory_create_items(main_item_factory, SIZE(playmenu_items),
      playmenu_items, NULL);
#else
    gtk_item_factory_create_items(main_item_factory, nplaymenu_items,
      playmenu_items, NULL);
#endif
    ...
    gtk_item_factory_create_items(main_item_factory, SIZE(helpmenu_items),
      helpmenu_items, NULL);
#else
    gtk_item_factory_create_items(main_item_factory, nhelpmenu_items,
      helpmenu_items, NULL);
#endif
...
#else
    nh_menu_sensitive(MENU_GAME MENU_PLAY, FALSE);
#endif
    nh_option_lock(TRUE);
...
GTK_putstr(winid id, int attr, const char *str)
{
	...
	case NHW_TEXT:
...
#else
		GTK_j_text_buffer_insert(t, &iter, buf, len);
#endif
	    else {
		if (attr == ATR_ULINE)
		    tag = "uline";
		...
		gtk_text_buffer_insert_with_tags_by_name(t, &iter, buf, len,
		  tag, NULL);
#else
		GTK_j_text_buffer_insert_with_tags_by_name(t, &iter, buf, len,
		  tag);
#endif
	    }
	    if (buf != str)
...
#else
	gchar *utf8_message = g_locale_to_utf8( "文書用フォント", -1, NULL, NULL, NULL );
	SetFont = gtk_font_selection_dialog_new(utf8_message);
	g_free( utf8_message );
#endif
...
GTK_ext_display_file(int fh)
{
    ...
    label = nh_gtk_new_and_pack(gtk_label_new("HELP"), vbox, "",
      FALSE, FALSE, NH_PAD);
#else
    label = nh_gtk_new_and_pack(GTK_j_label_new("ヘルプ"), vbox, "",
      FALSE, FALSE, NH_PAD);
#endif
...
#else
	    help_font_name = g_strdup("ms pgothic");
#endif
...
#else
    button = nh_gtk_new_and_add(GTK_j_button_new_with_mnemonic("フォントの選択(_S)..."),
#endif
      bbox, "");
...
#else
	GTK_j_text_buffer_insert_at_cursor(buffer, buf, strlen(buf));
#endif
#else
#if 0 /*JP*/
	gtk_text_insert(GTK_TEXT(text), NULL, NULL, NULL, buf, strlen(buf));
#else
	GTK_j_text_insert(GTK_TEXT(text), NULL, NULL, NULL, buf, strlen(buf));
#endif
#endif
    }
...
#else /*JP*/
    nh_menu_sensitive(MENU_GAME MENU_PLAY, TRUE);
    nh_menu_sensitive(MENU_GAME MENU_SAVE, FALSE);
    nh_menu_sensitive(MENU_MOVE, FALSE);
    nh_menu_sensitive(MENU_FIGH, FALSE);
    nh_menu_sensitive(MENU_CHEC, FALSE);
    nh_menu_sensitive(MENU_EQUI, FALSE);
    nh_menu_sensitive(MENU_YOU, FALSE);
    nh_menu_sensitive(MENU_ADVE, FALSE);
    nh_menu_sensitive(MENU_ACTI, FALSE);
    nh_menu_sensitive(MENU_RELI, FALSE);
    nh_menu_sensitive(MENU_SPEC, FALSE);
#endif /*JP*/
...
#else /*JP*/
    nh_menu_sensitive(MENU_GAME MENU_PLAY, FALSE);
    nh_menu_sensitive(MENU_GAME MENU_SAVE, TRUE);
    nh_menu_sensitive(MENU_MOVE, TRUE);
    nh_menu_sensitive(MENU_FIGH, TRUE);
    nh_menu_sensitive(MENU_CHEC, TRUE);
    nh_menu_sensitive(MENU_EQUI, TRUE);
    nh_menu_sensitive(MENU_YOU, TRUE);
    nh_menu_sensitive(MENU_ADVE, TRUE);
    nh_menu_sensitive(MENU_ACTI, TRUE);
    nh_menu_sensitive(MENU_RELI, TRUE);
    nh_menu_sensitive(MENU_SPEC, TRUE);
#endif /*JP*/
    nh_option_lock(TRUE);
...
# endif
...
#else
    if ( player ) {
	utf8_str = g_locale_to_utf8(player, -1, NULL, NULL, NULL);
	Sprintf(mstr, "%s", utf8_str);
	g_free(utf8_str);
    } else {
	Sprintf(mstr, "Rodney");
    }
    ...
    total_len = gdk_mbstowcs(wstr, utf8_str, NH_BUFSIZ);
    g_free(utf8_str);
#endif
    line = DEATH_LINE;
    wc = wstr;
...
void
GTK_j_text_buffer_insert(GtkTextBuffer *buffer, GtkTextIter *iter,
	const gchar *text, gint len)
{
    gchar *utf8_message = g_locale_to_utf8( text, len, NULL, NULL, NULL );
    gtk_text_buffer_insert(buffer, iter, utf8_message, -1);

    g_free( utf8_message );
}

void
GTK_j_text_buffer_insert_at_cursor(GtkTextBuffer *buffer,
	const gchar *text, gint len)
{
    gchar *utf8_message = g_locale_to_utf8( text, len, NULL, NULL, NULL );
    gtk_text_buffer_insert_at_cursor(buffer, utf8_message, -1);
    g_free( utf8_message );
}

void
GTK_j_text_buffer_insert_with_tags_by_name(GtkTextBuffer *buffer,
	GtkTextIter *iter, const gchar *buf, gint len, const gchar *tag)
{
    gchar *utf8_message = g_locale_to_utf8( buf, len, NULL, NULL, NULL );
    gtk_text_buffer_insert_with_tags_by_name(buffer, iter, utf8_message, -1, tag, NULL);
    g_free( utf8_message );
}

#ifndef USE_TEXTVIEW
void
GTK_j_text_insert(GtkText *text, GdkFont *font, GdkColor *fore,
	GdkColor *back, const gchar *chars, gint length)
{
    gchar *utf8_message = g_locale_to_utf8( chars, length, NULL, NULL, NULL );
    gtk_text_insert(GTK_TEXT(text), font, fore, back, utf8_message, -1);
...
void
GTK_j_label_set_text(GtkLabel *label, const char *str)
{
    gchar *utf8_message = g_locale_to_utf8( str, -1, NULL, NULL, NULL );
    gtk_label_set_text(label, utf8_message);
    g_free( utf8_message );
}

void
GTK_j_frame_set_label(GtkFrame *frame, const char *label)
{
    gchar *utf8_message = g_locale_to_utf8( label, -1, NULL, NULL, NULL );
    gtk_frame_set_label(frame, utf8_message);
    g_free( utf8_message );
}

void
GTK_j_window_set_title(GtkWindow *parent, const gchar *label)
{
    gchar *utf8_message = g_locale_to_utf8( label, -1, NULL, NULL, NULL );
    gtk_window_set_title(parent, utf8_message);
    g_free( utf8_message );
}

void
GTK_j_entry_set_text(GtkEntry *entry, const gchar *text)
{
    gchar *utf8_message = g_locale_to_utf8( text, -1, NULL, NULL, NULL );
    gtk_entry_set_text(entry, utf8_message) ;
    g_free( utf8_message );
}

void
GTK_j_notebook_set_tab_label_text(GtkNotebook *notebook, GtkWidget *child, const gchar *tab_text)
{
    gchar *utf8_message = g_locale_to_utf8( tab_text, -1, NULL, NULL, NULL );
    gtk_notebook_set_tab_label_text(notebook, child, utf8_message);
    g_free( utf8_message );
}

GtkWidget*
GTK_j_frame_new(const gchar *label)
{
    GtkWidget *widget ;
    if ( label == NULL ) {
	...
	gchar *utf8_message = g_locale_to_utf8( label, -1, NULL, NULL, NULL );
	widget = gtk_frame_new(utf8_message);
	g_free( utf8_message );
    }
    return widget ;
}
...
GtkWidget*
GTK_j_label_new(const gchar *label)
{
    GtkWidget *widget ;
    if ( label == NULL ) {
	...
	gchar *utf8_message = g_locale_to_utf8( label, -1, NULL, NULL, NULL );
	widget = gtk_label_new(utf8_message);
	g_free( utf8_message );
    }
...
GtkWidget*
GTK_j_label_new_with_mnemonic(const gchar *label)
{
    GtkWidget *widget ;
    gchar *utf8_message = g_locale_to_utf8( label, -1, NULL, NULL, NULL );

    widget = gtk_label_new_with_mnemonic(utf8_message);
    g_free( utf8_message );
...
GtkWidget*
GTK_j_button_new_with_label(const gchar *label)
{
    GtkWidget *widget ;
    if ( label == NULL ) {
	...
	gchar *utf8_message = g_locale_to_utf8( label, -1, NULL, NULL, NULL );
	widget = gtk_button_new_with_label(utf8_message);
	g_free( utf8_message );
    }
...
GtkWidget*
GTK_j_button_new_with_mnemonic(const gchar *label)
{
    GtkWidget *widget ;
    gchar *utf8_message = g_locale_to_utf8( label, -1, NULL, NULL, NULL );

    widget = gtk_button_new_with_mnemonic(utf8_message);
    g_free( utf8_message );
...
GtkWidget*
GTK_j_check_button_new_with_label(const gchar *label)
{
    GtkWidget *button;
    gchar *utf8_message = g_locale_to_utf8( label, -1, NULL, NULL, NULL );

    button = gtk_check_button_new_with_label(utf8_message);
    g_free( utf8_message );
...
GtkWidget*
GTK_j_check_button_new_with_mnemonic(const gchar *label)
{
    GtkWidget *button;
    gchar *utf8_message = g_locale_to_utf8( label, -1, NULL, NULL, NULL );

    button = gtk_check_button_new_with_mnemonic(utf8_message);
    g_free( utf8_message );
...
GtkWidget*
GTK_j_radio_button_new_with_label(GSList *group, const gchar *label)
{
    GtkWidget *button;
    gchar *utf8_message = g_locale_to_utf8( label, -1, NULL, NULL, NULL );

    button = gtk_radio_button_new_with_label(group, utf8_message);
    g_free( utf8_message );
...
GtkWidget*
GTK_j_radio_button_new_with_mnemonic(GSList *group, const gchar *label)
{
    GtkWidget *button;
    gchar *utf8_message = g_locale_to_utf8( label, -1, NULL, NULL, NULL );

    button = gtk_radio_button_new_with_mnemonic(group, utf8_message);
    g_free( utf8_message );
...
GtkWidget*
GTK_j_menu_item_new_with_label(const gchar *label)
{
    GtkWidget *menu_item;
    gchar *utf8_message = g_locale_to_utf8( label, -1, NULL, NULL, NULL );

    menu_item = gtk_menu_item_new_with_label(utf8_message);
    g_free( utf8_message );
...
GtkWidget*
GTK_j_menu_item_new_with_mnemonic(const gchar *label)
{
    GtkWidget *menu_item;
    gchar *utf8_message = g_locale_to_utf8( label, -1, NULL, NULL, NULL );

    menu_item = gtk_menu_item_new_with_mnemonic(utf8_message);
    g_free( utf8_message );
...
GtkWidget*
GTK_j_file_selection_new(const gchar *title)
{
    GtkWidget *file_selection;
    gchar *utf8_message = g_locale_to_utf8( title, -1, NULL, NULL, NULL );

    file_selection = gtk_file_selection_new(utf8_message);
    g_free( utf8_message );
...
GtkItemFactoryEntry *
G_j_locale_to_utf8_menu(int itemsize, GtkItemFactoryEntry *locale_items)
{
    int i;
    GtkItemFactoryEntry *result_items = (GtkItemFactoryEntry *)
	alloc(sizeof(GtkItemFactoryEntry) * itemsize);

    for (i = 0; i < itemsize; i++) {
	result_items[i].path
	    = g_locale_to_utf8( locale_items[i].path, -1, NULL, NULL, NULL );
	result_items[i].accelerator = locale_items[i].accelerator;
	result_items[i].callback = locale_items[i].callback;
	result_items[i].callback_action = locale_items[i].callback_action;
	result_items[i].item_type = locale_items[i].item_type;
	result_items[i].extra_data = locale_items[i].extra_data;
    }
...
create_utf8_menu()
{
    if ( mainmenu_items == NULL ) {
	nmainmenu_items = SIZE(locale_mainmenu_items);
	mainmenu_items = G_j_locale_to_utf8_menu(nmainmenu_items
	    , locale_mainmenu_items);
    }
    if ( helpmenu_items == NULL ) {
	nhelpmenu_items = SIZE(locale_helpmenu_items);
	helpmenu_items = G_j_locale_to_utf8_menu(nhelpmenu_items
	    , locale_helpmenu_items);
    }
    if ( playmenu_items == NULL ) {
	nplaymenu_items = SIZE(locale_playmenu_items);
	playmenu_items = G_j_locale_to_utf8_menu(nplaymenu_items
	    , locale_playmenu_items);
    }
}
...
destroy_utf8_menu()
{
    int i;
    if ( mainmenu_items != NULL ) {
	for (i = 0; i < nmainmenu_items; i++) {
	    g_free(mainmenu_items[i].path);
	}
	nmainmenu_items = 0;
	free(mainmenu_items);
	mainmenu_items = NULL;
    ...
    if ( helpmenu_items != NULL ) {
	for (i = 0; i < nhelpmenu_items; i++) {
	    g_free(helpmenu_items[i].path);
	}
	nhelpmenu_items = 0;
	free(helpmenu_items);
	helpmenu_items = NULL;
    ...
    if ( playmenu_items != NULL ) {
	for (i = 0; i < nplaymenu_items; i++) {
	    g_free(playmenu_items[i].path);
	}
	nplaymenu_items = 0;
	free(playmenu_items);
	playmenu_items = NULL;
