/*
  $Id: gtkmisc.c,v 1.6 2009/07/21 15:38:09 so-miya Exp $
 */
/*
...
#else /*JP*/
    {"(わかっているなら)ペットを攻撃するのを避ける", "はい", "いいえ", "safe_pet"},
    {"ペットや友好的な怪物を攻撃する前に確認する", "はい", "いいえ", "confirm"},
#ifdef TEXTCOLOR
    {"ペットを強調表示する", "はい", "いいえ", "hilite_pet"},
#endif
    {NULL,},
    {"経験値を表示する", "はい", "いいえ", "showexp"},
#ifdef SCORE_ON_BOTL
    {"スコアを表示する", "はい", "いいえ", "showscore"},
#endif
    {"ゲームの経過時間を表示する", "はい", "いいえ", "time"},
    {NULL,},
    {"移動したとき自動で物を拾う", "はい", "いいえ", "autopickup"},
    {NULL,},
    {"ゲームの最初にイントロダクションを表示する", "はい", "いいえ", "legacy"},
#ifdef NEWS
    {"スタートアップ時にゲーム管理者からのメッセージを表示する．", "はい", "いいえ", "news"},
#endif
#ifdef MAIL
    {"メールデーモンの存在を有効にする", "はい", "いいえ", "mail"},
#endif
    {NULL,},
    {"スペースバーでも休憩する", "はい", "いいえ", "rest_on_space"},
    {"ゲームのコメントを詳しく表示する", "はい", "いいえ", "verbose"},
    {NULL,},
    {"死んだとき墓石を表示する", "はい", "いいえ", "tombstone"},
    {NULL,},
    {"できるだけ，同じ物に同じ文字を割り当てる", "はい", "いいえ", "fixinv"},
    {"所持品を種類別に表示する", "はい", "いいえ", "sortpack"},
#endif /*JP*/
};
...
nh_option_more_confirm(void)
{
    ...
    nh_gtk_new_and_pack(gtk_label_new("Commit changes made?"),
      hbox, "", FALSE, FALSE, 0);
#else
    nh_gtk_new_and_pack(GTK_j_label_new("変更を反映しますか？"),
      hbox, "", FALSE, FALSE, 0);
#endif
...
#else
    GTK_j_entry_set_text(GTK_ENTRY(entry_plname), nh_option_cache_get("name"));
    GTK_j_entry_set_text(GTK_ENTRY(entry_catname), nh_option_cache_get("catname"));
    GTK_j_entry_set_text(GTK_ENTRY(entry_dogname), nh_option_cache_get("dogname"));
    GTK_j_entry_set_text(GTK_ENTRY(entry_fruit), nh_option_cache_get("fruit"));
#endif
...
#else
nh_option_cache_set(char *option, const char *utf8_value)
#endif
{
...
nh_option_plname_new()
{
    ...
    label = nh_gtk_new_and_pack(gtk_label_new("Name:"), hbox, "",
      FALSE, FALSE, NH_PAD);
#else
    label = nh_gtk_new_and_pack(GTK_j_label_new("名前:"), hbox, "",
      FALSE, FALSE, NH_PAD);
#endif
...
nh_option_pet_kitten_new()
{
    ...
    label = nh_gtk_new_and_pack(gtk_label_new("Name:"),
      hbox, "", FALSE, FALSE, NH_PAD);
#else
    label = nh_gtk_new_and_pack(GTK_j_label_new("名前:"),
      hbox, "", FALSE, FALSE, NH_PAD);
#endif
...
nh_option_pet_new()
{
    ...
    label = nh_gtk_new_and_pack(gtk_label_new("Start with:"), hbox2, "",
      FALSE, FALSE, NH_PAD);
#else
    label = nh_gtk_new_and_pack(GTK_j_label_new("初期ペット:"), hbox2, "",
      FALSE, FALSE, NH_PAD);
#endif
...
#else
    radio_k = nh_gtk_new_and_pack(
      GTK_j_radio_button_new_with_label(pet_group, "子猫"), hbox2, "",
      FALSE, FALSE, NH_PAD);
#endif
...
#else
    radio_d = nh_gtk_new_and_pack(
      GTK_j_radio_button_new_with_label(pet_group, "仔犬"), hbox2, "",
      FALSE, FALSE, NH_PAD);
#endif
...
#else
    radio_r = nh_gtk_new_and_pack(
      GTK_j_radio_button_new_with_label(pet_group, "ランダム"), hbox2, "",
      FALSE, FALSE, NH_PAD);
#endif
...
nh_option_fruit_new()
{
    ...
    label = nh_gtk_new_and_pack(gtk_label_new("Name:"),
      hbox, "", FALSE, FALSE, NH_PAD);
#else
    label = nh_gtk_new_and_pack(GTK_j_label_new("名称:"),
      hbox, "", FALSE, FALSE, NH_PAD);
#endif
...
#else
    frame = GTK_j_frame_new("メニュースタイル");
#endif
...
nh_option_game_new()
{
	...
	if (p->opt_name) {
	    ...
	    ltmp = nh_gtk_new_and_pack(gtk_label_new(p->opt_name), htmp, "",
	      FALSE, FALSE, NH_PAD);
#else
	    ltmp = nh_gtk_new_and_pack(GTK_j_label_new(p->opt_name), htmp, "",
	      FALSE, FALSE, NH_PAD);
#endif
...
#else
	    p->radio1 = nh_gtk_new_and_pack(
	      GTK_j_radio_button_new_with_label(p->group, p->on), htmp, "",
	      FALSE, FALSE, 0);
#endif
...
#else
	    p->radio2 = nh_gtk_new_and_pack(
	      GTK_j_radio_button_new_with_label(p->group, p->off), htmp, "", 
	      FALSE, FALSE, 0);
#endif
...
#else
	tileTab[0].data = nh_gtk_new_and_pack(
	  GTK_j_radio_button_new_with_label(menu_group, "文字表示"), hbox, "", 
	  FALSE, FALSE, NH_PAD);
#endif
...
#else
	    tileTab[i].data = nh_gtk_new_and_pack(
	      GTK_j_radio_button_new_with_label(menu_group, tileTab[i].ident),
	      hbox, "", FALSE, FALSE, NH_PAD);
#endif
	    menu_group =
...
nh_option_new()
{
...
#else
    GTK_j_notebook_set_tab_label_text(GTK_NOTEBOOK(note),
      gtk_notebook_get_nth_page(GTK_NOTEBOOK(note), 0), "プレイヤー");
#endif
...
#else
    GTK_j_notebook_set_tab_label_text(GTK_NOTEBOOK(note),
      gtk_notebook_get_nth_page(GTK_NOTEBOOK(note), 1), "ゲーム");
#endif
...
#else
    GTK_j_notebook_set_tab_label_text(GTK_NOTEBOOK(note),
      gtk_notebook_get_nth_page(GTK_NOTEBOOK(note), 2), "その他");
#endif
    ...
    button3 = nh_gtk_new_and_pack(gtk_button_new_with_label("More Options"),
      hbox, "", FALSE, FALSE, NH_PAD);
#else
    button3 = nh_gtk_new_and_pack(GTK_j_button_new_with_label("追加オプション"),
      hbox, "", FALSE, FALSE, NH_PAD);
#endif
