/*
  $Id: gtkstatus.c,v 1.5 2005/05/12 14:43:03 so-miya Exp $
 */
/*
...
#else
    {STAT_BAR(1),	"体",		"hp",		"hpmax"},
    {STAT_BAR(2),	"魔",		"pw",		"pwmax"},
    {STAT_COLUMN(1),	"鎧",   	"ac",		NULL},
    {STAT_COLUMN(1),	"金貨",  	"gold",		NULL},
    {STAT_COLUMN(1),	"レベル",	"elevel",	NULL},
    {STAT_COLUMN(1),	"HD",		"hitdice",	NULL},
    {STAT_COLUMN(1),	"属性",		"align",	NULL},
    {STAT_COLUMN(1),	"TIME",		"time",		NULL},
    {STAT_COLUMN(1),	"経験",		"experience",	NULL},
    {STAT_COLUMN(1),	"スコア",	"score",	NULL},
    {STAT_COLUMN(1),	"荷重",		"weight",	"capacity"},
    {STAT_COLUMN(2),	"強",		"strength",	NULL},
    {STAT_COLUMN(2),	"早",		"dexterity",	NULL},
    {STAT_COLUMN(2),	"耐",		"constitution",	NULL},
    {STAT_COLUMN(2),	"知",		"intelligence",	NULL},
    {STAT_COLUMN(2),	"賢",		"wisdom",	NULL},
    {STAT_COLUMN(2),	"魅",		"charisma",	NULL},
#endif
#if defined(MONITOR_HEAP) && defined(INTERNAL_MALLOC)
	...
	for(j = 0; j < nv; j++) {
		...
		if (k >=0 && k < STAT_COLS) {
		    ...
		    gtk_label_set_text(stat_widgets[k][rowno[k]++].label,
		      stat_tab[i].label);
#else
		    GTK_j_label_set_text(stat_widgets[k][rowno[k]++].label,
		      stat_tab[i].label);
#endif
		}
	...
	if (!stat_tab[i].oldvalue || strcmp(str, stat_tab[i].oldvalue)) {
		...
		case STAT_COLUMN(2):
		    ...
		    gtk_label_set_text(stat_widgets[j][stat_tab[i].row].value,
		      str);
#else
		    GTK_j_label_set_text(stat_widgets[j][stat_tab[i].row].value,
		      str);
#endif
...
#else
		    GTK_j_label_set_text(GTK_LABEL(bar[j].lbl), str);
#endif
		    break;
		case STAT_FRAME:
#if 0 /*JP*/
...
#else
		    GTK_j_frame_set_label(GTK_FRAME(frame), str);
#endif
		    break;
		case STAT_DLEVEL:
#if 0 /*JP*/
...
#else
		    GTK_j_label_set_text(GTK_LABEL(dlvl), str);
#endif
		    break;
		case STAT_HUNGER:
#if 0 /*JP*/
...
#else
		    GTK_j_label_set_text(GTK_LABEL(hung), str);
#endif
		    break;
...
#else
		    GTK_j_label_set_text(GTK_LABEL(encu), str);
#endif
		    break;
		case STAT_FLAGS:
		    sscanf(value, "%lX", &fl);
...
#else
		    GTK_j_label_set_text(GTK_LABEL(levi),
		      (fl & RAW_STAT_LEVITATION) ? " 浮遊 " : "");
#endif
#if 0 /*JP*/
		    gtk_label_set_text(GTK_LABEL(blin),
		      (fl & RAW_STAT_BLIND) ? " 盲目 " : "");
#else
		    GTK_j_label_set_text(GTK_LABEL(blin),
...
#else
		    GTK_j_label_set_text(GTK_LABEL(conf),
		      (fl & RAW_STAT_CONFUSION) ? " 混乱 " : "");
#endif
#if 0 /*JP*/
		    gtk_label_set_text(GTK_LABEL(sick),
		      ...
		      (fl & RAW_STAT_ILL) ? " Ill " : "");
#else
		    GTK_j_label_set_text(GTK_LABEL(sick),
		      (fl & RAW_STAT_FOODPOIS) ? " 食毒 " :
		      (fl & RAW_STAT_ILL) ? " 病気 " : "");
#endif
#if 0 /*JP*/
...
#else
		    GTK_j_label_set_text(GTK_LABEL(stun),
		      (fl & RAW_STAT_STUNNED) ? " 眩暈 " : "");
#endif
#if 0 /*JP*/
		    gtk_label_set_text(GTK_LABEL(hall),
		      (fl & RAW_STAT_HALLUCINATION) ? " Hallucination " : "");
#else
		    GTK_j_label_set_text(GTK_LABEL(hall),
		      (fl & RAW_STAT_HALLUCINATION) ? " 幻覚 " : "");
#endif
#if 0 /*JP*/
		    gtk_label_set_text(GTK_LABEL(slim),
		      (fl & RAW_STAT_SLIMED) ? " Slimed " : "");
#else
		    GTK_j_label_set_text(GTK_LABEL(slim),
		      (fl & RAW_STAT_SLIMED) ? " 溶解 " : "");
#endif
		    break;
	    }
	    g_free(stat_tab[i].oldvalue);
...
#else
	bar[i].lbl = nh_gtk_new_and_pack(GTK_j_label_new(stat_tab[j].label),
	  bar[i].hbox, "", FALSE, FALSE, 0);
#endif
