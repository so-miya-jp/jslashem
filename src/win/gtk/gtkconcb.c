/* $Id: gtkconcb.c,v 1.5 2005/05/12 14:43:40 so-miya Exp $ */
...
GTK_create_connection(GtkWidget *dialog, gpointer user_data)
{
	...
	gchar *utf8_message = g_locale_to_utf8(
	  "新しい接続名を入力してください．", -1, NULL, NULL, NULL );
	w = gtk_message_dialog_new(GTK_WINDOW(dialog),
	  GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
	  utf8_message);
	g_free(utf8_message);
...
#else
	{
	    gchar *utf8_message = g_locale_to_utf8(
	      "接続名%sは既に接続されています．\n\n"
	      "別の接続名を入力してください．", -1, NULL, NULL, NULL );
	    w = gtk_message_dialog_new(GTK_WINDOW(dialog),
	      GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
	      utf8_message, name);
	    g_free(utf8_message);
...
#else
    w = GTK_j_file_selection_new("ローカル実行環境の選択");
#endif
...
#else
	gchar *utf8_message = g_locale_to_utf8(
	  "サーバが接続待機を行っているTCP/IPポートを入力してください．",
	  -1, NULL, NULL, NULL );
	w = gtk_message_dialog_new(GTK_WINDOW(RemoteExecutable),
	  GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
	  utf8_message);
...
#else
	gchar *utf8_message = g_locale_to_utf8(
	  "TCP/IPポート番号が正しくありません．\n\n"
	  "接続可能なTCP/IPポートを入力してください．",
	  -1, NULL, NULL, NULL );
	w = gtk_message_dialog_new(GTK_WINDOW(RemoteExecutable),
	  GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
	  utf8_message);
...
#else
	gchar *utf8_message = g_locale_to_utf8(
	  "ファイル'%s'は既に存在します．", -1, NULL, NULL, NULL );
	dialog = gtk_message_dialog_new(GTK_WINDOW(fs),
	   GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_WARNING,
	   GTK_BUTTONS_NONE, utf8_message, filename);
	g_free(utf8_message);
#endif
...
#else
	gchar *utf8_message = g_locale_to_utf8(
	  "ファイル'%s'への保存に失敗しました：%s", -1, NULL, NULL, NULL );
	dialog = gtk_message_dialog_new(GTK_WINDOW(fs),
	   GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR,
	   GTK_BUTTONS_CLOSE, utf8_message,
	   filename, g_strerror(errno));
	g_free(utf8_message);
#endif
...
#else
	GtkWidget *SaveAs = GTK_j_file_selection_new("接続ロゴの保存");
#endif
