/*
  $Id: gtkprefs.c,v 1.7 2005/05/12 14:43:03 so-miya Exp $
 */
/*
...
#else
    radar_button = GTK_j_check_button_new_with_label("レーダー表示を行う");
#endif
...
GTK_pref_map_new()
{
    ...
    frame = nh_gtk_new_and_pack(gtk_frame_new("Automatic scrolling"), vbox, "",
      FALSE, FALSE, NH_PAD);
#else
    frame = nh_gtk_new_and_pack(GTK_j_frame_new("自動スクロール"), vbox, "",
      FALSE, FALSE, NH_PAD);
#endif
    ...
    nh_gtk_new_and_pack(gtk_label_new("Maximum allowable deviation:"), hbox, "",
      FALSE, FALSE, NH_PAD);
#else
    nh_gtk_new_and_pack(GTK_j_label_new("最大許容値:"), hbox, "",
      FALSE, FALSE, NH_PAD);
#endif
...
GTK_preferences_new()
{
...
#else
    GTK_j_window_set_title(GTK_WINDOW(w), DEF_GAME_NAME " カスタマイズ");
#endif
    ...
    GTK_j_notebook_set_tab_label_text(GTK_NOTEBOOK(note),
      gtk_notebook_get_nth_page(GTK_NOTEBOOK(note), 0), "一般");
#endif
...
#else
    GTK_j_notebook_set_tab_label_text(GTK_NOTEBOOK(note),
      gtk_notebook_get_nth_page(GTK_NOTEBOOK(note), 1), "マップ");
#endif
