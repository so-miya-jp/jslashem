/*
  $Id: gtkmessage.c,v 1.4 2005/05/12 14:43:03 so-miya Exp $
 */
/*
...
nh_message_putstr(const char *str)
{
	...
	gtk_text_buffer_insert_with_tags_by_name(t, &iter, buf, len + 1,
	  "warning", NULL);
#else
	GTK_j_text_buffer_insert_with_tags_by_name(t, &iter, buf, len + 1,
	  "warning");
#endif
    else
#if 0 /*JP*/
...
#else
	GTK_j_text_buffer_insert(t, &iter, buf, len + 1);
#endif
    free(buf);
...
#else
    GTK_j_text_insert(t, NULL, &nh_color[i], &nh_color[MAP_WHITE], buf, len + 1);
#endif
