...
STATIC_DCL int FDECL(tty_role_select, (char *, char *));
STATIC_DCL int FDECL(tty_race_select, (char *, char *));
...
# pragma unused(argcp,argv)
    ...
    tty_putstr(BASE_WINDOW, 0, JA_COPYRIGHT_BANNER_E);
    tty_putstr(BASE_WINDOW, 0, JA_COPYRIGHT_BANNER_F);
    tty_putstr(BASE_WINDOW, 0, "");
#endif
...
#else
						"中断", MENU_UNSELECTED);
				Sprintf(pbuf, "%s性別を選んでください", plbuf);
#endif
				end_menu(win, pbuf);
...
#else
						"中断", MENU_UNSELECTED);
				Sprintf(pbuf, "%s属性を選んでください", plbuf);
#endif
				end_menu(win, pbuf);
				n = select_menu(win, PICK_ONE, &selected);
				destroy_nhwindow(win);
