...
#ifdef BETA
# ifdef PORT_SUB_ID
		"%s JSLASH'EM %s Beta Version %d.%d.%d-%d - last build %s.",
# else
		"%s JSLASH'EM Beta Version %d.%d.%d-%d - last build %s.",
# endif
#else
# ifdef PORT_SUB_ID
		"%s JSLASH'EM %s Version %d.%d.%d - last build %s.",
# else
		"%s JSLASH'EM Version %d.%d.%d - last build %s.",
# endif
#endif
		PORT_ID,
#ifdef PORT_SUB_ID
...
#else
	infile = malloc(strlen(DATA_IN_TEMPLATE) - 2 + strlen(DATA_FILE) + 7);
#endif
...
#else
	Sprintf(infile, DATA_IN_TEMPLATE, "j" DATA_FILE);
#endif
	Strcat(infile,
#ifdef SHORT_FILENAMES
		".bas"
...
do_japanese(void)
{
    ...
    unsigned char buf[BUFSZ], key[BUFSZ], val[BUFSZ];
    int i, hval, rhval;
...
/*    fprintf(fpw,"  const int type;\n");*/
	...
	i = 0;
	while(fgets((char *)buf, BUFSZ, fpr) != NULL){
	    if(split_string(buf, key, val)){
		hval = hash_val(key);
		rhval = hash_val(val);
