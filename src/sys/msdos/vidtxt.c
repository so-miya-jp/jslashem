...
#if 1 /*JP*/
	if (!CO) txt_get_scr_size();
	if (!attr) attr = ATTRIB_NORMAL;
#endif
	txt_get_cursor(&col,&row);
	switch(ch) {
	    case '\n':	
...
#   endif
	...
	if (row >= LI) {
		row = LI-1;
		regs.h.ah = SCROLL;     /* scroll up    */
		regs.h.al = 1;          /* scroll line  */
		regs.h.bh = 0;          /* display page */
		regs.h.ch = 0;          /* area top     */
		regs.h.cl = 0;          /* area left    */
		regs.h.dh = LI-1;       /* area bottom  */
		regs.h.dl = CO-1;       /* area right   */
		(void) int86(VIDEO_BIOS, &regs, &regs);
	}
#endif
	txt_gotoxy(col,row);
